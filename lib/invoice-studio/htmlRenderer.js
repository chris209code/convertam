...
import { formatMoney, fromCents } from './moneyFormat';
import { computeItemLine } from './calculations';
...
function renderLetterhead(data) {
  ...
  return `<div><img src="${esc(data.src)}" style="width:100%;display:block;" /></div>`;
}
...
function itemCellValue(colId, row, currency) {
  ...
  switch (colId) {
    ...
    case 'amount': return cellMoney(fromCents(computeItemLine(row).lineTotalCents));
  ...
  const rows = data.rows.map((row, i) => {
    ...
    return `
      <tr>
        ...
        ${showImages ? `<td style="${cellStyle}">${row.img ? `<img src="${esc(row.img)}" style="width:32px;height:32px;object-fit:cover;border-radius:6px;" />` : ''}</td>` : ''}
        <td style="${cellStyle}">
...
function renderSignatureBlock(data, label, s, anchorRight) {
  ...
  const mark = isTyped
    ...
    : (data.src ? `<img src="${esc(data.src)}" style="height:${size}px;width:auto;max-width:100%;object-fit:contain;" />` : '');
