...
import { GEMINI_MODEL, GEMINI_MODEL_URL } from './geminiModel';
 ...
 * @param {number} [opts.temperature] - omit to use Gemini's own default
 * @param {number} [opts.timeoutMs] - per-attempt request timeout; omit for no
 *   explicit bound (relies solely on the serverless platform's own function
 *   timeout, today's default behavior for every existing caller)
 ...
 */
export async function callGemini({ apiKey, toolName, routeName, parts, schema, maxOutputTokens, temperature, timeoutMs, inputSizeApprox, hasImage }) {
  const requestId = generateRequestId();
  const model = GEMINI_MODEL;
  const url = GEMINI_MODEL_URL;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    const timeoutHandle = timeoutController ? setTimeout(() => timeoutController.abort(), timeoutMs) : null;
    let res, data;
    try {
      res = await fetch(url, {
        method: 'POST',
      ...
      logAttempt({ requestId, toolName, routeName, model, attempt, maxAttempts, intendedGeminiCalls: 1, actualGeminiAttempts: attempt, success: false, durationMs: Date.now() - start, note: isTimeout ? `timeout_after_${timeoutMs}ms` : `network_error: ${networkErr.message}`, inputSizeApprox, hasImage });
      lastError = isTimeout
        ? new AIError('timeout', requestId, `No response within ${timeoutMs}ms`)
        : new AIError('transient', requestId, networkErr.message);
      if (!isTimeout && attempt < maxAttempts) { await sleep(jitter(1000)); continue; }
      throw lastError;
    } finally {
    ...
    // A schema-constrained response that hits the token cap mid-generation
    // can still be syntactically valid JSON (the model just stops emitting
    // further array elements) — so JSON.parse below would happily succeed
    // on a SILENTLY INCOMPLETE result rather than throwing. Catching
    // MAX_TOKENS explicitly here is what actually prevents that: without
    // it, a caller has no way to distinguish "the whole thing" from "as
    // much as fit," which is exactly how a dense 3-minute transcription
    // chunk lost several minutes of real, spoken dialogue with no error
    // ever surfaced — the truncated segment list just looked complete.
    if (finishReason === 'MAX_TOKENS') {
      logAttempt({ requestId, toolName, routeName, model, finishReason, attempt, maxAttempts, durationMs, success: false, note: 'max_tokens_truncated', usage, requestedMaxOutputTokens: maxOutputTokens || null, inputSizeApprox, hasImage });
      lastError = new AIError('unexpected', requestId, 'Response was truncated at the output token limit before it finished');
