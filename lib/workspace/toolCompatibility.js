...
export const PULL_ONLY_TOOLS = ['split-pdf'];
...
// payment-gated server jobs, and the office conversions / pdf-to-png either
// run through CloudConvert or produce a non-PDF result. Per the architecture
// doc's compatibility rule, image-output tools (pdf-to-png) never pretend to
// hand a PDF back into the session — they're listed here specifically so the
// sidebar can mark them "opens with a new upload" instead of implying
// continuity that doesn't exist yet.
export const DESTINATION_ONLY_TOOLS = [
  'protect-pdf', 'ocr-pdf', 'compress-pdf',
  'pdf-to-word', 'pdf-to-excel', 'pdf-to-powerpoint', 'pdf-to-png',
];
