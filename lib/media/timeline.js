...
// Bulk delete for multi-select — a single pure-function pass rather than
// looping deleteClip() per id (looping would also work, since each call
// renormalizes order safely, but doing it in one pass keeps this an atomic
// operation with no intermediate states). clipIds may span multiple
// tracks; each affected track's order is renormalized once. Removing
// clips only ever shortens a track's sequence — since there's no
// independent position field, this can never leave a gap or overlap.
export function deleteClips(timeline, clipIds) {
  const idSet = new Set(clipIds);
  const affectedTracks = new Set(timeline.clips.filter((c) => idSet.has(c.id)).map((c) => c.track));
  if (!affectedTracks.size) return timeline;
  let clips = timeline.clips.filter((c) => !idSet.has(c.id));
  for (const track of affectedTracks) clips = renormalizeOrder(clips, track);
...
// track order, preserving the selected clips' own relative sequence
// within that block. clipIds must all belong to the same track (the UI
// only ever builds a multi-selection within one track — see
// VideoEditorWorkspace's click handlers); clips from any other track are
// ignored rather than erroring, so a stray id can't corrupt the result.
export function duplicateClips(timeline, clipIds) {
  ...
  const lastIndex = trackClips.findIndex((c) => c.id === selected[selected.length - 1].id);
  const copies = selected.map((c) => ({ ...c, id: nextId('clip') }));
  const newTrackClips = [...trackClips.slice(0, lastIndex + 1), ...copies, ...trackClips.slice(lastIndex + 1)];
  const others = timeline.clips.filter((c) => c.track !== track);
  return { ...timeline, clips: [...others, ...newTrackClips.map((c, i) => ({ ...c, order: i }))] };
}
...
// range), found by the same cumulative-duration walk findActiveClipAt()
// uses. A static image has no bounded end (it fills the rest of its
// track), so `end` is Infinity for those. Used by the trim-handle
// snapping UI to know a clip's fixed timeline-start (unaffected by
// trimming that same clip — see VideoEditorWorkspace's own comment on
// handleTrimHandleMove for why).
export function getClipTimelineBounds(timeline, clipId) {
  const clip = timeline.clips.find((c) => c.id === clipId);
  if (!clip) return null;
  let elapsed = 0;
  for (const c of getTrackClips(timeline, clip.track)) {
    const source = timeline.sources.find((s) => s.id === c.sourceId);
    const isStatic = source && source.kind === 'image';
    const dur = isStatic ? Infinity : clipDuration(c);
    if (c.id === clipId) return { start: elapsed, end: elapsed + dur };
    elapsed += dur;
  }
  return null;
}
...
export function getAllClipBoundaryTimes(timeline) {
  const times = [];
  for (const track of [MAIN_TRACK, ...timeline.overlayTracks.map((t) => t.id)]) {
    let elapsed = 0;
    for (const c of getTrackClips(timeline, track)) {
      const source = timeline.sources.find((s) => s.id === c.sourceId);
      const isStatic = source && source.kind === 'image';
      const dur = isStatic ? Infinity : clipDuration(c);
      times.push(elapsed);
      if (Number.isFinite(dur)) times.push(elapsed + dur);
      elapsed += dur;
    }
  }
  return times;
