...
// Templates are skins, not different invoices — switching from Modern to
// Corporate must change the visual layout/colors only, never discard what
// was already entered. This takes the NEW template's fresh layout skeleton
// (correct positions/styling for that skin) and merges in the CURRENT
// document's actual content field-by-field, per element kind. New elements
// the target template introduces that didn't exist before use the skin's
// own defaults, since there's nothing to preserve for them.
export function applyTemplateSkin(currentElements, newTemplateId) {
  const skeleton = buildTemplateElements(newTemplateId);
  const currentById = Object.fromEntries((currentElements || []).map((e) => [e.id, e]));

  return skeleton.map((skinEl) => {
    const existing = currentById[skinEl.id];
    if (!existing) return skinEl;

    switch (skinEl.kind) {
      case 'logo': return { ...skinEl, src: existing.src ?? skinEl.src };
      case 'letterhead': return { ...skinEl, src: existing.src ?? skinEl.src, visible: existing.src ? existing.visible : skinEl.visible };
      case 'companyText': return { ...skinEl, name: existing.name ?? skinEl.name, tagline: existing.tagline ?? skinEl.tagline };
      case 'contactInfo': return { ...skinEl, phone: existing.phone, email: existing.email, website: existing.website, address: existing.address };
      case 'billTo': return { ...skinEl, clientName: existing.clientName, clientAddress: existing.clientAddress, clientPhone: existing.clientPhone };
      case 'meta': return { ...skinEl, rows: skinEl.rows.map((r) => { const ex = existing.rows?.find((er) => er.key === r.key); return ex ? { ...r, value: ex.value } : r; }) };
      case 'table': return { ...skinEl, rows: existing.rows?.length ? existing.rows : skinEl.rows, showImages: existing.showImages ?? skinEl.showImages };
      case 'notes': return { ...skinEl, content: existing.content ?? skinEl.content };
      case 'terms': return { ...skinEl, content: existing.content ?? skinEl.content, visible: existing.visible ?? skinEl.visible };
      case 'payment': return { ...skinEl, methods: existing.methods ?? skinEl.methods, visible: existing.visible ?? skinEl.visible };
      case 'bank': return { ...skinEl, rows: skinEl.rows.map((r, i) => (existing.rows?.[i] ? { ...r, v: existing.rows[i].v } : r)), visible: existing.visible ?? skinEl.visible };
      case 'qr': return { ...skinEl, value: existing.value ?? skinEl.value, src: existing.src ?? skinEl.src, visible: existing.visible ?? skinEl.visible };
      case 'approval': return { ...skinEl, name: existing.name ?? skinEl.name, role: existing.role ?? skinEl.role };
      case 'signature': return { ...skinEl, mode: existing.mode, text: existing.text, src: existing.src, visible: existing.visible ?? skinEl.visible };
      case 'stamp': return { ...skinEl, src: existing.src, opacity: existing.opacity ?? skinEl.opacity, visible: existing.visible ?? skinEl.visible };
      case 'watermark': return { ...skinEl, content: existing.content ?? skinEl.content, visible: existing.visible ?? skinEl.visible };
      default: return { ...skinEl, visible: existing.visible ?? skinEl.visible };
    }
  });
}
