...
// Underline has no real PDF text layer to snap to (pages are rendered
// bitmaps) — like highlight, the user drags out a rectangular region and the
// line is drawn along its baseline. The rect itself stays the hit-test
// target (not just the thin line) so clicking anywhere in the underlined
// region selects it, matching how highlight/strikethrough behave.
...
export function draw(ctx, o) {
  ...
  ctx.beginPath();
  const y = o.y + o.h - (o.thickness ?? DEFAULT_THICKNESS) / 2;
  ctx.moveTo(o.x, y);
