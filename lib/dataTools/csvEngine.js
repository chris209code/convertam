...
export function rowsToTable(rawRows, { hasHeader = true } = {}) {
  ...
  // Math.max(...array) blows the call stack on large imports (tens of
  // thousands of rows) — reduce() has no such argument-count limit.
  const width = rawRows.reduce((max, r) => (r.length > max ? r.length : max), 0);
...
export function columnStats(table, column) {
  ...
  if (type === 'integer' || type === 'decimal') {
    ...
    if (nums.length) {
      // Math.min/max(...array) blows the call stack on large numeric
      // columns — reduce() has no such argument-count limit.
      stats.min = nums.reduce((a, b) => (b < a ? b : a), nums[0]);
      stats.max = nums.reduce((a, b) => (b > a ? b : a), nums[0]);
