...
// The background replacement choices offered after removal. White is the
// default for nearly every preset; a preset whose own published guidance
// calls for something other than pure white (UK, Schengen) defaults to
// Light Grey instead, per that guidance.
export const BACKGROUND_OPTIONS = [
  { id: 'transparent', label: 'Transparent' },
  { id: 'white', label: 'White', hex: '#FFFFFF' },
  { id: 'light-grey', label: 'Light grey', hex: '#F1F0EC' },
  { id: 'light-blue', label: 'Light blue', hex: '#DCEEFB' },
  { id: 'custom', label: 'Custom colour' },
];

export function defaultBackgroundOption(preset) {
  const hex = (preset.background.hex || '').toUpperCase();
  if (hex === '#F1F0EC' || hex === '#F5F5F5') return 'light-grey';
  return 'white';
}
