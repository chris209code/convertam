...
// Per-role bullets only — exp.achievements (a CV Improver addition) is
// never rendered here. It gets its own standalone "Achievements" section,
// the same level as Education/Skills/Certifications, grouped by role (see
// groupAchievementsByRole + the Achievements section in each body builder
// below) rather than nested under each individual job.
function expBulletsHtml(exp, fallbackLines) {
  return bulletsHtml(fallbackLines);
}

// Mirrors resumeTemplates.js's groupAchievementsByRole — one group per role
// that actually has achievements, in experience order, each with its own
// bullets, so the standalone Achievements section reads the same way in
// the Word export as it does in the PDF.
function groupAchievementsByRole(experience) {
  return (experience || [])
    .map((exp) => ({
      role: exp.role || '',
      company: exp.company || '',
      items: (Array.isArray(exp.achievements) ? exp.achievements : []).filter(Boolean),
    }))
    .filter((group) => group.items.length > 0);
}
function achievementGroupsHtml(groups) {
  return groups.map((group) => {
    const heading = [group.role, group.company].filter(Boolean).join(' — ');
    return `<p><strong>${esc(heading)}</strong></p>${bulletsHtml(group.items)}`;
  }).join('\n');
}
...
function renderLegacyBody(data) {
  ...
  if (achievementGroups.length) {
    ...
    parts.push(achievementGroupsHtml(achievementGroups));
  }
  if (skills.length) {
...
// data.achievements is groupAchievementsByRole()'s output (one group per
// role, from adaptToModernProfessionalData) — reuses achievementGroupsHtml
// so the standalone section reads identically to the legacy body's.
function modernAchievementsHtml(groups = []) {
  return achievementGroupsHtml(groups);
}
...
function renderMpCenteredBody(data) {
  ...
  if (data.achievements?.length) { parts.push('<h2>Achievements</h2>'); parts.push(modernAchievementsHtml(data.achievements)); }
...
function renderMpSidebarBody(data) {
  ...
  if (data.achievements?.length) mainParts.push(`<h2>Achievements</h2>${modernAchievementsHtml(data.achievements)}`);
...
function renderMpHeaderBandBody(data) {
  ...
  if (data.achievements?.length) leftParts.push(`<h2>Achievements</h2>${modernAchievementsHtml(data.achievements)}`);
