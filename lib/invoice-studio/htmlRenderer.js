...
// ONE flex row, matching the live editor's PaymentBankSection — Signature
// used to be a separate block stacked below Payment/Bank here (the PDF path
// is a separate implementation from the editor, so it hadn't picked up the
// same fix applied there).
function renderPaymentBank(payment, bank, signature, s) {
  if (!payment.visible && !bank.visible && !signature?.visible) return '';
  const head = fontCss(s.headingFont), body = fontCss(s.bodyFont);
  ...
  if (signature?.visible) {
    ...
    const mark = isTyped
      ...
      : (signature.src ? `<img src="${signature.src}" style="max-height:40px;object-fit:contain;" />` : '');
  ...
  return `<div style="display:flex;gap:32px;align-items:flex-start;">${paymentHtml}${bankHtml}${signatureHtml}</div>`;
}

// QR is normal flow content, same as the editor's QrSection — it used to
// be position:absolute pinned to the page's bottom edge, which meant it had
// no idea how tall the real content above it was and could land on top of
// Terms & Conditions once that section pushed the actual content further
// down than the QR's fixed offset accounted for. Flowing normally means it
// always renders below whatever's above it and pushes Terms down in turn.
function renderQr(data) {
  if (!data?.visible || !data.src) return '';
  return `<div style="width:68px;height:68px;"><img src="${esc(data.src)}" style="width:100%;height:100%;object-fit:contain;" /></div>`;
}
...
export function renderInvoiceHtml(doc, s, totals, wordsText) {
  ...
  const body = `
    <div style="position:relative;z-index:0;width:794px;min-height:1123px;background:#fff;padding:40px;box-sizing:border-box;display:flex;flex-direction:column;">
      ...
      <div style="margin-top:22px; break-inside:avoid;">${renderPaymentBank(sec.payment, sec.bank, sec.signature, s)}</div>
      <div style="margin-top:22px; break-inside:avoid;">${renderQr(sec.qr)}</div>
