...
import { drawCompositionFrame, getComposeSize } from './compositionLayouts';
...
export async function renderComposedTimeline(timeline, { onStatus, onProgress }) {
  ...
  const { width: composeWidth, height: composeHeight } = getComposeSize(timeline.frameAspect);
...
// The composed (canvas) path is also required, even with no overlay at
// all, whenever a non-default frame shape is chosen: reframing a single
// landscape source into square/vertical means cropping every frame into a
// differently-shaped canvas, which only the composed pipeline can do — the
// simple path just trims/concats the source's own native frame shape.
export async function renderTimeline(timeline, callbacks) {
  ...
  return (hasOverlay || needsReframe) ? renderComposedTimeline(timeline, callbacks) : renderSimpleTimeline(timeline, callbacks);
}
