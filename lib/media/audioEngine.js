...
// Reuses lib/media/audioCleanup.js's buildCleanupFilterChain (the same
// live-graph noise-reduction chain Screen Recorder already uses on its mic
// input) rather than inventing a second one.
//
...
import { buildCleanupFilterChain } from './audioCleanup';
import { clipDuration, isTrackAudible } from './audioTimeline';

// Maps a getUserMedia rejection's DOMException name to a specific, actionable
// message — shared by every tool that records a microphone (Recording
// Studio, Voice Over) so a permission denial, a missing device, and a
// device already in use each get their own real explanation instead of one
// generic "could not access the microphone" for every case.
...
// Live microphone capture for recording, with the same optional real-time
// noise-reduction filter chain Screen Recorder already applies to its own
// mic input (buildCleanupFilterChain) — so what gets RECORDED is already
// cleaned, not just what's monitored. Rejects with the browser's own
// error name (NotAllowedError, NotFoundError, etc.) so the caller can show
// a clear, specific permission/hardware message rather than a generic one.
export async function startMicCapture({ ctx, liveCleanup = false, cleanupIntensity = 'medium' }) {
  ...
  if (liveCleanup) {
    ...
    const output = buildCleanupFilterChain(ctx, micSource, { intensity: cleanupIntensity });
    output.connect(dest);
    recordStream = dest.stream;
    cleanupNodes = { micSource, dest };
  }
  ...
  return { mediaRecorder, micStream: stream, cleanupNodes };
}

export function stopMicCapture({ micStream, cleanupNodes }) {
  micStream.getTracks().forEach((t) => t.stop());
  if (cleanupNodes) {
    try { cleanupNodes.micSource.disconnect(); } catch { /* no-op */ }
  }
}
