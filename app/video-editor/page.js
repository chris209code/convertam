...
const FAQ = [
  ...
  { q: 'Can I split a clip into two?', a: 'Yes — click the clip exactly where you want to cut (that moves the white cut line there and selects the clip), then click Split. The clip becomes two independent clips you can trim, delete, or reorder separately. Split and Delete buttons sit right under the timeline for quick access.' },
  { q: 'Can I delete part of a video?', a: 'Yes — click the clip where the unwanted part starts and click Split, then click where it ends and Split again, then click that middle piece and Delete it. The remaining clips automatically play back to back with no gap.' },
  ...
  { q: 'Can I upload several videos at once instead of one at a time?', a: 'Yes — the video uploader accepts multiple files in a single selection (or a single drag-and-drop of several files). They\'re added to the main track in the order given, and each one remains fully independent afterward — select, trim, split, reorder, or delete any of them exactly as if you\'d added it on its own.' },
  { q: 'When I join several videos together, how do I know where I actually am in each original file?', a: 'A time badge rides right on the playhead line as it crosses the main track — e.g. "0:42.3 · src 5:18.1" — so when it sits inside the third clip of a five-video timeline, you can read off exactly which moment of THAT clip\'s own original file you\'re looking at, down to a tenth of a second, without looking away from the timeline you\'re about to cut. It never disappears, and keeps tracking during playback as well as while scrubbing. Each clip\'s label on the timeline strip also shows its own source range for the same reason, which makes choosing a precise split point on a merged multi-video timeline much easier.' },
  { q: 'Can I preview my edit fullscreen?', a: 'Yes — click the Fullscreen button in the top-right corner of the preview to expand it to fill your screen, so you can see exactly what your export will look like before you export it.' },
  ...
  { q: 'Is a video\'s audio stuck to its video clip, or can I edit them separately?', a: 'They\'re separate by default. Every main-track video clip with its own audio automatically gets a matching clip on a dedicated Audio track directly under the main track, with its own waveform. Trimming, splitting, moving, reordering, or deleting the video clip carries the audio along with it automatically — but you can just as easily select the audio clip itself on the Audio track and split, trim, move, or delete it independently, without touching the video at all. The moment you edit the audio clip directly, it stops following its video clip going forward (its label switches from a 🔗 linked badge to "Independent"); editing the video clip never breaks the sync unless you\'ve done that first.' },
  ...
  { q: 'Can I change a clip\'s speed, or add fade in/out?', a: 'Yes — every clip has a Speed control (0.25× to 4×) and Fade in/Fade out. For a main-track video clip with its own audio (which lives on its own linked clip on the Audio track — see below), an Apply to Video only / Audio only / Both choice next to Speed controls whether a change affects the picture, the sound, or both together, so a slow-motion shot doesn\'t have to slow its narration down too.' },
  { q: 'Are there transitions between clips?', a: 'Cut, Crossfade/Dissolve, Fade to Black, Fade to White, Wipe, Slide, Push, Zoom, Blur Dissolve, and Circle/Iris Reveal are all available from the selected clip\'s Transition control, each with a duration you pick from 0.25s/0.5s/1s/2s/3s/5s/7s/10s. Crossfade, Wipe, Slide, Push, Zoom, Blur Dissolve, and Circle/Iris Reveal genuinely blend both clips\' video together (not just a fade through a color), with the incoming clip held on its own first frame during the blend so there\'s no jump once its own slot begins. Preview updates immediately as you pick a transition or change its duration, and choosing Cut removes it. For the very start and end of the whole video, use the Composition panel\'s Opening/Closing transitions to fade in or out from black or white — no clip selection needed.' },
  ...
  { q: 'Can I add my own music or sound effects to a video?', a: 'Yes — the Audio tab\'s Sound clips section lets you add as many independent audio clips as you like: background music, sound effects, extra narration. Each upload lands as its own draggable clip right on the timeline — drag to reposition, drag its edges to trim, or select it and use Split/Delete like any other clip — plus its own Volume and Mute, and clips can freely overlap each other. This is separate from the Audio Track (used for pairing a whole replacement audio track with your video — also splittable/trimmable once added) and from a clip\'s own Replace/Mix audio.' },
  ...
  { q: 'Can I add captions to my video?', a: 'Yes — click Auto Captions at any point while editing (no export needed first). It renders your edited timeline\'s audio locally, transcribes it, and lets you fix up the transcript or download it as SRT, VTT, or TXT. Turn on "Show in preview & export" in the Caption style & position panel and your captions appear live on the canvas and are included the next time you click the regular Export MP4 button — there\'s no separate "burn" step.' },
  { q: 'I already have a subtitle file — can I just use that, without transcription?', a: 'Yes — Burn Subtitles (in the Captions panel, below Auto Captions) is a separate, no-AI path: upload your own .srt or .vtt file and it becomes the active caption source for the exact same style/position controls and "Show in preview & export" toggle Auto Captions uses. Nothing is uploaded anywhere for this — it works entirely from the file you give it.' },
  { q: 'Can I style my captions — font, color, background, highlight, karaoke?', a: 'Yes — the Caption style & position panel has seven starting presets (Classic, Bold, Minimal, Social, Highlight, Karaoke, Pop) plus full manual control: font size and weight, text color, a separate highlight color, background box with its own color and opacity, outline, shadow, overall opacity, line spacing, letter spacing, and an entrance animation (fade/pop/rise). Word-by-word and karaoke-style highlighting recolor each word as playback reaches it, using an even per-word timing estimate (V1 has no real per-word transcription timestamps to work from yet) rather than actual speech detection.' },
  { q: 'Can I move my captions anywhere on the video, not just top/middle/bottom?', a: 'Yes — drag the caption directly on the preview to place it anywhere, and drag the handles on either side of its text box to make it wider or narrower (which controls where the text wraps, not the font size). Left/Center/Right alignment controls how multi-line text sits within that box. All of this is drawn straight onto the same canvas the export captures, so what you see in the preview is exactly what ends up in your exported video — no separate, less-accurate burn-in step to second-guess.' },
  { q: 'Can I clean up noisy audio on a clip?', a: 'Yes — open a clip\'s audio controls and click "Clean audio" to reduce hum/buzz, rumble, and hiss, with optional voice clarity and volume normalization. It\'s plain signal processing (not AI), runs locally in your browser, and replaces that clip\'s audio in place — it won\'t remove every kind of noise, but it genuinely helps.' },
  { q: 'Does adding captions upload my video?', a: 'No — never your video, and not even your original audio files. Auto Captions renders your edited timeline\'s audio mix locally first, then sends a compressed copy of just that rendered audio to our transcription provider for that one request; it\'s not stored afterward. Drawing and exporting captions happens locally in your browser, like every other part of the timeline.' },
  ...
  { q: 'Do I need to export before generating captions?', a: 'No — that\'s the point. Auto Captions works directly off your current timeline, and captions preview live on the canvas as soon as you turn them on. Export only happens once, whenever you click the regular Export MP4 button — the same export whether or not captions are showing.' },
          ...
          <article style={{ maxWidth: 760, margin: '0 auto', color: '#334155', lineHeight: 1.75, fontSize: '0.95rem' }}>
            ...
            <section style={{ marginBottom: 36 }}>
              ...
              <ul style={ul}>
                ...
                <li><strong>Split</strong> — click a clip exactly where you want to cut, then click Split (also available right under the timeline). To remove a section from the middle, split at both edges of the part you don&apos;t want, then select that piece and delete it. On a timeline made of several joined videos, a time badge riding right on the playhead line (and each clip&apos;s own timeline label) always shows exactly where it sits in that clip&apos;s own original file, down to a tenth of a second, not just the combined timeline, for a precise split point.</li>
                <li><strong>Multi-file upload</strong> — select or drag in several videos at once instead of adding them one at a time; they land on the main track in order, each fully independent afterward.</li>
                ...
                <li><strong>Close gap</strong> — adding a second video always starts it right where the first one ends, so playback flows straight through with no pause; if a drag (or any other edit) leaves a gap before a clip, a one-click warning snaps it flush again.</li>
                ...
                <li><strong>Fullscreen preview</strong> — expand the preview to fill your screen to see exactly what your export will look like, without the timeline and panels around it.</li>
                ...
                <li><strong>Speed</strong> — 0.25× to 4× per clip, with an Apply to Video only / Audio only / Both choice for a main-track clip whose audio lives on its own linked Audio-track clip — so the picture and the sound don&apos;t have to share one speed.</li>
                <li><strong>Fade in/out</strong> — per clip, video and audio together.</li>
                <li><strong>Transitions</strong> — cut, crossfade/dissolve, fade to black, fade to white, wipe, slide, push, zoom, blur dissolve, or circle/iris reveal between clips, each with a duration you pick from 0.25s up to a full 10s, plus a separate opening/closing fade (to or from black/white) for the very start and end of the video.</li>
                <li><strong>Audio track</strong> — every main-track video clip with its own audio gets a matching clip on a dedicated Audio track underneath, with its own waveform, that stays in sync with the video by default (trim/split/move/reorder/delete on either side carries over automatically) but can just as easily be selected and edited on its own — trim, split, move, or delete it independently the moment you touch it directly.</li>
                <li><strong>Sound clips</strong> — add as many independent audio clips as you like (music, sound effects, extra narration), each with its own position, length, volume, and mute, freely overlapping the others.</li>
                ...
                <li><strong>Auto Captions</strong> — transcribe your edited timeline directly (no export needed first), edit the transcript, and download SRT/VTT/TXT.</li>
                <li><strong>Burn Subtitles</strong> — already have an .srt or .vtt file? Upload it — no transcription, no AI, nothing uploaded — and it becomes another caption source for the same styling below.</li>
                <li><strong>Caption style &amp; position</strong> — seven presets (Classic, Bold, Minimal, Social, Highlight, Karaoke, Pop) plus full control over font size/weight, text and highlight color, background box, outline, shadow, opacity, line/letter spacing, and an entrance animation; word-by-word and karaoke highlighting recolor each word as playback reaches it. Drag the caption anywhere on the preview to position it and drag its side handles to resize the text box (which controls wrapping, not font size), with Left/Center/Right alignment — drawn on the same canvas the export captures, so preview and export always match exactly. One "Show in preview &amp; export" toggle includes whichever source is active in your regular export — no separate burn step.</li>
                <li><strong>Clean audio</strong> — reduce hum, rumble, and hiss on a clip&apos;s audio right from its audio controls, with optional voice clarity and normalization, before it's mixed into your final export.</li>
