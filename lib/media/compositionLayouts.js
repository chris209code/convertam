...
// { ctx, timeline, mainEl, overlayEl, mainClip, overlayClip, mainOpacity,
// overlayOpacity, rounded, border } — mainEl is always a <video>; overlayEl
// may be a <video> or a static <img> (image overlay). mainClip/overlayClip
// (the timeline.js clip objects, not elements) supply per-clip filters and
// crop focus; mainOpacity/overlayOpacity (0..1, default 1) are the fade
// multiplier from getFadeOpacity() — callers compute it once and pass the
// number through rather than this function reaching back into timeline
// state, keeping this module's only dependency on a clip shape minimal.
export function drawCompositionFrame(ctx, { timeline, mainEl, overlayEl, mainClip, overlayClip, mainOpacity = 1, overlayOpacity = 1, rounded = false, border = false, canvasWidth, canvasHeight }) {
  ...
  if (rects.overlay && overlayEl) {
    ...
    if (timeline.compositionMode === 'pip' && rounded) {
      ...
      ctx.clip();
      fillRectBg(r);
      ctx.filter = buildFilterString(overlayClip?.filters);
      ctx.globalAlpha = overlayOpacity;
      drawInto(overlayEl, r, overlayClip);
      ctx.restore();
    } else {
      ctx.save();
      fillRectBg(r);
