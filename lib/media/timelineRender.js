...
import { drawCompositionFrame, drawTextOverlays, drawImageOverlays, drawShapeOverlays, drawWatermark, drawCaptions, getComposeSize, getFadeOpacity, buildCutoutCanvas } from './compositionLayouts';
...
export async function renderComposedTimeline(timeline, { onStatus, onProgress, cancelToken, captionEvents }) {
  ...
  // Project audio (see timeline.js's projectAudio) — a single, clip-
  // independent audio bed loaded once up front (same reasoning as the
  // image elements above) so it's ready to start the instant the recording
  // itself does, rather than being lazily loaded on first use the way a
  // per-clip element is (a clip's own audio only needs to be ready once
  // that clip becomes active; this needs to be ready at t=0 always).
  const projectAudioSource = timeline.projectAudio?.sourceId ? timeline.sources.find((s) => s.id === timeline.projectAudio.sourceId) : null;
  const projectAudioEl = projectAudioSource ? document.createElement('audio') : null;
  if (projectAudioEl) {
    projectAudioEl.src = URL.createObjectURL(projectAudioSource.file);
    await new Promise((resolve) => { projectAudioEl.oncanplaythrough = resolve; projectAudioEl.onerror = resolve; });
  }
  ...
  const projectAudioGain = audioCtx.createGain();
  if (projectAudioEl) {
    projectAudioGain.gain.value = timeline.projectAudio.muted ? 0 : (timeline.projectAudio.gain ?? 1);
    audioCtx.createMediaElementSource(projectAudioEl).connect(projectAudioGain);
  }
  ...
  // Started exactly once, right alongside the recording itself, and never
  // paused/reseeked afterward -- unlike every per-clip audio element above,
  // project audio isn't scoped to "whichever clip is active"; it just plays
  // from its own beginning for as long as it naturally lasts (or until the
  // frame loop below ends the render, whichever comes first).
  if (projectAudioEl) { projectAudioEl.currentTime = 0; await projectAudioEl.play().catch(() => {}); }
  ...
  if (projectAudioEl) { projectAudioEl.pause(); if (projectAudioEl.src) URL.revokeObjectURL(projectAudioEl.src); }
...
export async function renderTimelineAudio(timeline, { onStatus, onProgress, cancelToken }) {
  ...
  // Auto Captions transcribes exactly this render, so a project audio bed
  // (see timeline.js's projectAudio) has to be included here too — without
  // it, a timeline whose actual speech lives entirely in a synced audio
  // track rather than any clip's own audio would render as near-silence,
  // and transcription would find nothing to caption.
  const projectAudioSource = timeline.projectAudio?.sourceId ? timeline.sources.find((s) => s.id === timeline.projectAudio.sourceId) : null;
  const projectAudioEl = projectAudioSource ? document.createElement('audio') : null;
  if (projectAudioEl) {
    projectAudioEl.src = URL.createObjectURL(projectAudioSource.file);
  ...
  recorder.start();
  if (projectAudioEl) {
    projectAudioEl.currentTime = 0;
    await projectAudioEl.play().catch(() => {});
  }
  const startTime = performance.now();
  let lastMainClipId = null;
  ...
  if (mainReplaceAudioEl.src) URL.revokeObjectURL(mainReplaceAudioEl.src);
  if (projectAudioEl) {
    projectAudioEl.pause();
    if (projectAudioEl.src) URL.revokeObjectURL(projectAudioEl.src);
  }
  for (const layer of overlayLayers) {
    layer.videoEl.pause();
    layer.replaceAudioEl.pause();
...
export async function renderTimeline(timeline, callbacks) {
  ...
  // A project audio bed (see timeline.js's projectAudio) plays continuously
  // and independently of any clip — the simple/ffmpeg trim+concat path has
  // no concept of an audio source that isn't scoped to one clip, so this
  // forces the composed path exactly like an overlay track does.
  const needsProjectAudio = !!timeline.projectAudio?.sourceId;
  ...
  // Any number of independent sound clips (timeline.js's soundTracks) — same
  // reasoning as needsProjectAudio: the simple path has no concept of an
  // audio source outside the main track's own clips.
  ...
  const needsComposed = hasOverlay || needsReframe || needsClipEffects || needsOverlayLayers || needsMasterAudio || needsFitMode || needsGaps || needsProjectAudio || needsImageClips || needsSoundTracks || needsCaptions;
