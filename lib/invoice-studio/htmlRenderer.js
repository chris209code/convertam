...
function renderBankSignature(bank, signature, qr, s) {
  ...
  if (signature?.visible) {
    ...
    const approvedByLabel = signature.approvedName ? `
      <div style="font-family:${body};font-size:10px;color:${s.textMuted};text-transform:uppercase;letter-spacing:.04em;margin-bottom:8px;">Approved By</div>` : '';
  ...
  const body = `
    <div style="position:relative;z-index:0;width:714px;min-height:1043px;background:#fff;display:flex;flex-direction:column;">
      ...
      <div style="break-inside:avoid;">
        ...
        <div style="margin-top:22px;">${renderBankSignature(sec.bank, sec.signature, sec.qr, s)}</div>
