...
Return ONLY JSON in this shape: { "issues": [{ "section": "...", "issue": "...", "suggestion": "..." }], "score": { "completeness": 0-100, "clarity": 0-100, "relevance": 0-100, "impact": 0-100, "atsStructure": 0-100 } }
Score each dimension honestly based on what's actually there — do not default to high scores. Keep issue/suggestion text concise (1-2 sentences each). List at most 8 issues, prioritizing the most impactful ones.`;
}

async function callGemini(apiKey, prompt, { json = false } = {}) {
  const body = {
    contents: [{ role: 'user', parts: [{ text: prompt }] }],
    generationConfig: json ? { responseMimeType: 'application/json' } : { maxOutputTokens: 1024 },
  };
  ...
  if (!res.ok) {
    console.error('Gemini resume-ai error:', data);
...
export async function POST(request) {
  ...
  try {
    ...
    if (action === 'bullets') {
      const raw = await callGemini(apiKey, buildBulletsPrompt(payload), { json: true });
    ...
    if (action === 'summary') {
      const raw = await callGemini(apiKey, buildSummaryPrompt(payload));
      return Response.json({ summary: raw.trim() });
    }

    if (action === 'skills') {
      const raw = await callGemini(apiKey, buildSkillsPrompt(payload), { json: true });
      const clean = raw.replace(/```json|```/g, '').trim();
    ...
    if (action === 'refine') {
      const raw = await callGemini(apiKey, buildRefinePrompt(payload));
      return Response.json({ text: raw.trim() });
    }

    if (action === 'polish') {
      const raw = await callGemini(apiKey, buildPolishPrompt(payload), { json: true });
      const clean = raw.replace(/```json|```/g, '').trim();
