...
class Parser {
  ...
  parseSelect() {
    ...
    if (this.isKeyword('AS')) { this.next(); from.alias = this.identName(); }
    else if (this.peek() && this.peek().type === 'ident') from.alias = this.identName();
    ...
    if (this.isKeyword('JOIN') || this.isKeyword('INNER') || this.isKeyword('LEFT')) {
      ...
      if (this.isKeyword('AS')) { this.next(); table.alias = this.identName(); }
      else if (this.peek() && this.peek().type === 'ident') table.alias = this.identName();
...
function coerceComparable(v) {
  ...
  if (/^-?\d+(\.\d+)?$/.test(trimmed)) return Number(s);
  // Imported rows store every cell as a string (CSV/XLSX import, JSON
  // array-to-table), including booleans — so a TRUE/FALSE literal in a
  // WHERE clause (parsed as a real JS boolean) would otherwise never
  // === -match the imported "true"/"false" string, silently returning zero
  // rows regardless of the actual data.
