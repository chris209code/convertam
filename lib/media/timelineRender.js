...
import { getTrackClips, findActiveClipAt, getTotalDuration, MAIN_TRACK, clipDuration } from './timeline';
    ...
    async function frame() {
      ...
      for (const layer of overlayLayers) {
        ...
        if (hit && isImage) {
          ...
          await applyReplacementAudio(hit.clip, layer.replaceAudioEl, layer.replaceGain, layer.replaceSrcRef, layer.replaceLastRef);
          layer.gain.gain.value = 0; // no video audio track to keep regardless of mode
          drawnOverlayLayers.push({ trackId: layer.trackId, el: layer.imageEl, clip: hit.clip, opacity });
        } else if (hit) {
          ...
          const layerDuckGain = layerDuckEnvelope ? duckGainAtTime(layerDuckEnvelope.peaks, layerDuckEnvelope.sourceDuration, hit.sourceTime) : 1;
          ...
          layer.gain.gain.value = ((hit.clip.audioMode === 'keep' || mixKeepsOwnAudio) ? 1 : 0) * opacity * (hit.clip.gain ?? 1);
          drawnOverlayLayers.push({ trackId: layer.trackId, el: layer.videoEl, clip: hit.clip, opacity });
        } else {
...
export async function renderTimeline(timeline, callbacks) {
  ...
  const needsClipEffects = mainClips.some((c) => (c.speed || 1) !== 1 || c.fadeIn > 0 || c.fadeOut > 0 || !isDefaultFilters(c.filters) || c.reversed || (c.gain ?? 1) !== 1 || c.duckBackground);
