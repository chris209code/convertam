...
export async function remuxToMp4(webmBlob, { onProgress } = {}) {
  ...
  try {
    ...
    const code = await ffmpeg.exec(['-i', inName, '-c:v', 'libx264', '-preset', 'veryfast', '-c:a', 'aac', outName]);
...
export async function burnAssSubtitles({ videoFile, assText, onProgress }) {
  const ffmpeg = await getFFmpeg();
  const { fetchFile } = await import('@ffmpeg/util');
  ...
  try {
    ...
    const code = await ffmpeg.exec(['-i', inName, '-vf', `subtitles=${assName}:fontsdir=.`, '-c:a', 'copy', outName]);
