    ...
    async function frame() {
      ...
      for (const layer of overlayLayers) {
        ...
        if (hit && isImage) {
          ...
          const mixKeepsOwnAudio = await applyReplacementAudio(hit.clip, layer.replaceAudioEl, layer.replaceGain, layer.replaceSrcRef, layer.replaceLastRef);
...
export async function renderTimeline(timeline, callbacks) {
  ...
  const needsClipEffects = mainClips.some((c) => (c.speed || 1) !== 1 || c.fadeIn > 0 || c.fadeOut > 0 || !isDefaultFilters(c.filters) || c.reversed || (c.gain ?? 1) !== 1);
