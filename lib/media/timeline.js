...
export function addClip(timeline, sourceId, track = MAIN_TRACK) {
  ...
  const clip = {
    ...
    order,
    audioMode: source.hasAudio ? 'keep' : 'mute',
...
export function setClipAudioMode(timeline, clipId, audioMode) {
  return { ...timeline, clips: timeline.clips.map((c) => (c.id === clipId ? { ...c, audioMode } : c)) };
}
