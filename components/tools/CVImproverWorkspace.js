...
async function generateCVPdf(data) {
  ...
  page.drawText(data.name || 'Your Name', { x: margin, y: height - 30, size: 20, font: bold, color: rgb(1,1,1) });
  ...
  pages.forEach(p => {
    p.drawRectangle({ x: 0, y: 0, width, height: 25, color: blue });
    p.drawText('Generated by Convertam · convertam.app', { x: width / 2 - 90, y: 8, size: 7, font, color: rgb(1,1,1,0.6) });
  });
...
export default function CVImproverWorkspace() {
  ...
  async function handleDownloadPDF() {
    ...
    try {
      ...
      downloadBlob(new Blob([bytes], { type: 'application/pdf' }), `${structured.name || 'Improved-CV'}.pdf`);
    } catch (err) {
        ...
        <div style={{ marginTop: 24 }}>
          ...
          <div style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: 16, padding: 24, marginBottom: 16 }}>
            ...
            <div style={{ background: 'linear-gradient(135deg, #2563EB, #1D4ED8)', borderRadius: 12, padding: 20, marginBottom: 20, color: 'white' }}>
              <div style={{ fontSize: '1.3rem', fontWeight: 800, marginBottom: 4 }}>{structured.name || 'Your Name'}</div>
          ...
          <div className="actions">
            ...
            <button className="btn btn-ghost" onClick={() => { setStructured(null); setCvText(''); setStatus(''); }}>
              Improve Another CV
            </button>
          </div>
        </div>
