...
async function generateCVPdf(data) {
  ...
  if (contacts.length) page.drawText(contacts.join('  |  '), { x: margin, y: height - 70, size: 8, font, color: rgb(1,1,1,0.75), maxWidth: contentWidth });
  y = height - 110;
  ...
  if (data.experience?.length) {
    ...
    data.experience.forEach(exp => {
      ...
      y -= 14;
      if (exp.company) { page.drawText(exp.company, { x: margin, y, size: 9, font, color: gray }); y -= 14; }
  ...
  if (data.education?.length) {
    ...
    data.education.forEach(edu => {
      ...
      y -= 14;
      if (edu.institution) { page.drawText(edu.institution, { x: margin, y, size: 9, font, color: gray }); y -= 16; }
    });
  }
  ...
  if (data.skills?.length) {
    ...
    data.skills.forEach(skill => {
      ...
      if (sx + w > width - margin) { sx = margin; y -= 22; checkY(22); }
...
// Section component for clean preview
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      <div style={{
        ...
        borderBottom: '1.5px solid #BFDBFE', paddingBottom: 4, marginBottom: 12,
      }}>{title}</div>
      {children}
    </div>
  );
  ...
  return (
    <div className="panel">
      <div style={{ marginBottom: 16 }}>
        <label style={{ fontSize: '0.75rem', fontWeight: 600, color: '#475569', display: 'block', marginBottom: 4 }}>Target Job Title (optional)</label>
      ...
      <div style={{ marginBottom: 16 }}>
        <label style={{ fontSize: '0.75rem', fontWeight: 600, color: '#475569', display: 'block', marginBottom: 4 }}>Paste Your CV / Resume Text</label>
        ...
        <div style={{ marginTop: 24 }}>
          ...
          <div style={{ background: 'white', border: '1px solid #E2E8F0', borderRadius: 16, overflow: 'hidden', marginBottom: 16 }}>
            ...
            <div style={{ background: 'linear-gradient(135deg, #2563EB, #1D4ED8)', padding: '24px 28px' }}>
              <div style={{ fontSize: '1.4rem', fontWeight: 800, color: 'white', marginBottom: 4 }}>{structured.name}</div>
              {structured.title && <div style={{ fontSize: '0.9rem', color: 'rgba(255,255,255,0.85)', marginBottom: 8 }}>{structured.title}</div>}
            ...
            <div style={{ padding: '24px 28px' }}>
              ...
              {structured.summary && (
                <PreviewSection title="Professional Summary">
                  <p style={{ fontSize: '0.82rem', color: '#475569', lineHeight: 1.7, margin: 0 }}>{structured.summary}</p>
                </PreviewSection>
              )}
              ...
              {structured.experience?.length > 0 && (
                <PreviewSection title="Work Experience">
                  {structured.experience.map((exp, i) => (
                    <div key={i} style={{ marginBottom: 16, paddingLeft: 14, borderLeft: '2px solid #BFDBFE' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
                        <span style={{ fontSize: '0.88rem', fontWeight: 700, color: '#0F172A' }}>{exp.role}</span>
                      ...
                      {exp.bullets?.map((b, j) => (
                        <div key={j} style={{ fontSize: '0.78rem', color: '#475569', marginBottom: 4, display: 'flex', gap: 8 }}>
                          <span style={{ color: '#2563EB', flexShrink: 0, fontWeight: 700 }}>•</span>{b}
                        </div>
                      ))}
                    </div>
                  ))}
                ...
                <PreviewSection title="Skills">
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                    {structured.skills.map((skill, i) => (
                      <span key={i} style={{ fontSize: '0.75rem', padding: '4px 12px', borderRadius: 99, background: '#EFF6FF', color: '#2563EB', border: '1px solid #BFDBFE', lineHeight: 1.4 }}>{skill}</span>
                    ))}
                  </div>
                </PreviewSection>
              )}
          ...
          {downloaded && (
            <div style={{ marginTop: 16, padding: '16px 20px', borderRadius: 12, background: '#ECFDF5', border: '1px solid #A7F3D0' }}>
              ...
              <p style={{ fontSize: '0.78rem', color: '#059669', margin: 0 }}>Love it? Share Convertam with a friend — <strong>convertam.app</strong></p>
            </div>
          )}
        </div>
      )}
