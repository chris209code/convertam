...
export function runQualityChecks(objects, slidesMeta, theme) {
  ...
  slidesMeta.forEach((meta, slideIndex) => {
    ...
    // normalizeSlide.js substitutes a generic "Slide N" placeholder whenever
    // Gemini omits/empties a title, so a truly-empty meta.title never
    // reaches here in practice — matching that placeholder pattern too is
    // what makes this finding actually reachable.
    const isPlaceholderTitle = /^Slide \d+$/.test((meta.title || '').trim());
    if (!meta.title || !meta.title.trim() || isPlaceholderTitle) {
