...
// Owner-only feature gate — for a feature meant ONLY for the site owner
// (never a public "free tier + owner gets more" split like Data Analyst's
// daily limit below), a route just needs a plain yes/no. Reuses the exact
// same signed 'convertam_owner' cookie /api/owner-login already sets.
export function isOwnerRequest(cookieHeader) {
  const cookieSecret = process.env.CONVERTAM_OWNER_COOKIE_SECRET;
  if (!cookieSecret) return false;
  const cookies = parseCookies(cookieHeader);
  const token = cookies['convertam_owner'];
  const payload = token ? decodeSignedCookie(token, cookieSecret) : null;
  return !!payload?.owner;
}

// Pure, testable core of the Data Analyst daily-report-limit decision —
// deliberately isolated here rather than left inline in the route, so it
// can be exercised directly without needing a real Request object or a
// live server. This function is specific to ONE tool's usage tracking; it
// takes an explicit cookie name and is never called by, or shared with,
// any other AI tool's route.
export function computeDataAnalystUsageState({ cookieHeader, ownerCookieName, usageCookieName, cookieSecret, dailyLimit, todayOverride }) {
  const cookies = parseCookies(cookieHeader);
  const ownerToken = cookies[ownerCookieName];
  const ownerPayload = ownerToken && cookieSecret ? decodeSignedCookie(ownerToken, cookieSecret) : null;
  const isOwner = !!ownerPayload?.owner;

  const today = todayOverride || currentWATDateString();
  const usageToken = cookies[usageCookieName];
  const usagePayload = usageToken && cookieSecret ? decodeSignedCookie(usageToken, cookieSecret) : null;
  // Reset whenever the stored date doesn't match today's calendar day.
  const count = usagePayload?.date === today ? (usagePayload.count || 0) : 0;

  return { isOwner, today, count, remaining: Math.max(0, dailyLimit - count) };
}

// Same decision, scoped to Document Translator's own daily fair-use cap —
// kept as its own named function rather than a shared generic one, matching
// the deliberate one-function-per-tool convention above (a tool's usage
// tracking never doubles up on another tool's cookie/limit by accident).
export function computeDocumentTranslatorUsageState({ cookieHeader, ownerCookieName, usageCookieName, cookieSecret, dailyLimit, todayOverride }) {
  const cookies = parseCookies(cookieHeader);
  const ownerToken = cookies[ownerCookieName];
  const ownerPayload = ownerToken && cookieSecret ? decodeSignedCookie(ownerToken, cookieSecret) : null;
  const isOwner = !!ownerPayload?.owner;
