...
export default function InvoiceStudioWorkspace() {
  ...
  const handleDownload = useCallback(async (format) => {
    ...
    try {
      if (format === 'pdf') {
        // The PDF is now generated by a real headless browser rendering the
        // exact same layout as the live preview (renderElements), then
        // exporting to true PDF — the same principle CV/Resume already
        // uses successfully: one rendering engine as the single source of
        // truth, not a second hand-coded implementation to keep in sync.
        const pdfCtx = {
          currency: doc.currency, totals, wordsText, pageLabel: 'Page 1 of 1',
          brandPrimary: doc.brandPrimary, brandSecondary: doc.brandSecondary, brandAccent: doc.brandAccent,
          companyName: companyTextEl?.name || '', headingFontName: doc.headingFont, bodyFontName: doc.bodyFont,
        };
        const res = await fetch('/api/invoice-pdf', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ elements: renderElements, ctx: pdfCtx }),
        });
        if (!res.ok) {
