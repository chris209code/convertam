  ...
  async function handleGenerate() {
    ...
    try {
      ...
      page.drawText(biz.name || 'Your Business', {
        x: 50, y: height - 55, size: 20, font: fontBold, color: dark,
      });
      let bizY = height - 74;
      ...
      // INVOICE label
      page.drawText('INVOICE', { x: width - 190, y: height - 52, size: 30, font: fontBold, color: amber });
      page.drawText(`#${invoice.number}`, { x: width - 190, y: height - 72, size: 10, font: fontReg, color: gray });
      page.drawText(`Date: ${invoice.date}`, { x: width - 190, y: height - 86, size: 9, font: fontReg, color: gray });
      if (invoice.dueDate) {
        page.drawText(`Due: ${invoice.dueDate}`, { x: width - 190, y: height - 100, size: 9, font: fontReg, color: gray });
      }
      ...
      // Total box
      totY -= 35;
      page.drawRectangle({ x: cols.rate - 30, y: totY - 6, width: width - cols.rate - 18, height: 32, color: amber });
      page.drawText('TOTAL DUE', { x: cols.rate - 18, y: totY + 8, size: 11, font: fontBold, color: white });
      page.drawText(fmtPdf(total), { x: cols.amount, y: totY + 8, size: 12, font: fontBold, color: white });
