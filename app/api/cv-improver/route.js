...
====================================================
- PROFESSIONAL SUMMARY — edit the candidate's own summary text below IN PLACE: tighten sentences, fix grammar, strengthen weak verbs, cut filler and repetition${hasTarget ? ', and shift emphasis toward the target position' : hasJobDesc ? ', and shift emphasis toward the role inferred from the job description' : ''}. The result must still clearly read as a polished version of what the candidate themselves wrote — same voice, same sentence-level ideas, same things they chose to emphasize — not a wholesale rewrite in a different voice or structure. Do NOT default to a generic templated opening such as "Results-driven [X] with [Y] years of experience in [Z]..." or any other boilerplate formula — that pattern discards the candidate's actual wording instead of improving it, which is never the goal. The ONLY exception: if the candidate's summary is missing, a single sentence fragment, or genuinely too thin to edit, write a new one from the rest of the CV's real content instead of leaving it blank.
