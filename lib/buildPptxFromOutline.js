...
export async function buildPptxFromOutline(outline, { themeKey = 'modern', includeSources = false } = {}) {
  ...
  outline.slides.forEach((slideData) => {
    ...
    if (slideData.type === 'title') {
      ...
      slide.addText(slideData.title || '', {
        x: 0.5, y: 0.3, w: 9, h: 0.6, fontFace: theme.fontFace, fontSize: 22, bold: true, color: theme.titleColor,
      });
      slide.addShape('rect', { x: 0.5, y: 0.95, w: 9, h: 0.03, fill: { color: theme.accentColor } });
      if (slideData.imageDataUrl) {
        slide.addImage({ data: slideData.imageDataUrl, x: 0.9, y: 1.2, w: 8.2, h: 3.3 });
      }
      if (slideData.caption) {
        slide.addText(slideData.caption, { x: 0.6, y: 4.65, w: 8.8, h: 0.8, fontFace: theme.fontFace, fontSize: 13, color: theme.bodyColor });
      }
    } else {
      slideNum++;
      slide.background = { color: theme.bg };
