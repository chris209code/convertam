...
export function drawCompositionFrame(ctx, { timeline, mainEl, mainClip, mainOpacity = 1, crossfadeEl, crossfadeClip, crossfadeOpacity = 0, overlayLayers = [], backgroundImageEl, rounded = false, border = false, canvasWidth, canvasHeight }) {
  ...
  function drawInto(mediaEl, rect, clip) {
    ...
    else drawCover(ctx, mediaEl, effectiveRect, cropFocus);
    ctx.restore();
  }
  ...
  if (crossfadeEl && crossfadeOpacity > 0) {
    ctx.save();
    ctx.filter = buildFilterString(crossfadeClip?.filters);
    ctx.globalAlpha = crossfadeOpacity;
    drawInto(crossfadeEl, rects.main, crossfadeClip);
    ctx.restore();
  }
...
function drawOneTextOverlay(ctx, o, w, h) {
  ...
  if (o.background && o.background !== 'none') {
    ...
    ctx.globalAlpha = (o.opacity ?? 1) * bgOpacity;
