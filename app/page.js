    ...
    <main style={{ background: 'linear-gradient(180deg, #F8FAFF 0%, #FFFFFF 100%)' }} className="min-h-screen">
      ...
      <section style={{ background: '#EFF6FF', position: 'relative', overflow: 'hidden' }}>
        <div style={{
          ...
          background: 'linear-gradient(180deg, transparent, #F8FAFF)', zIndex: 1,
        }} />
        <div style={{
                ...
                <div className="grid md:grid-cols-2 gap-3">
                  {items.map(t => (
                    <Link key={t.slug} href={`/${t.slug}`}
                      className="flex items-center gap-4 p-4 rounded-2xl border transition-all hover:-translate-y-1 hover:shadow-md"
