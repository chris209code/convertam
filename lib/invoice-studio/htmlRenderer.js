...
// QR is positioned content, pinned to the page's own bottom-left corner, same
// as the editor's QrOverlay — it does not push other content down.
function renderQr(data) {
  if (!data?.visible || !data.src) return '';
  return `<div style="position:absolute;left:40px;bottom:70px;width:68px;height:68px;"><img src="${esc(data.src)}" style="width:100%;height:100%;object-fit:contain;" /></div>`;
}
