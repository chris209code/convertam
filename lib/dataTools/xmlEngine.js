...
// A lazy [\s\S]*? stops at the FIRST '>', which for a DOCTYPE with an
// internal subset (e.g. <!DOCTYPE x [<!ELEMENT x (#PCDATA)><!ENTITY xxe
// SYSTEM "...">]>) only captures up through the first declaration's own
// '>' — missing any ENTITY/SYSTEM declaration that follows it, and
// silently letting hasUnsafeDeclaration wave the whole thing through.
// Matching the optional internal subset explicitly (up to its closing ']')
// before the doctype's real closing '>' captures the entire declaration.
