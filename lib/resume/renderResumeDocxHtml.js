...
function modernExperienceHtml(experience = []) {
  return experience.map((exp) => {
    ...
    return `<p><strong>${esc(exp.role)}</strong>${dates ? ` (${esc(dates)})` : ''}<br/>${esc(exp.company)}${exp.location ? ` · ${esc(exp.location)}` : ''}</p>${bulletsHtml(exp.bullets)}`;
  }).join('\n');
}
