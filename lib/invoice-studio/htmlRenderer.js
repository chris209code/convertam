...
function renderBankSignature(bank, signature, qr, s, docType) {
  const config = docTypeConfig(docType);
  const bankVisible = bank.visible && config.showBank;
  const showQrHere = !bankVisible && qr?.visible && qr.src;
  if (!bankVisible && !signature?.visible && !showQrHere) return '';
  const head = fontCss(s.headingFont), body = fontCss(s.bodyFont);
  ...
  if (signature?.visible) {
    ...
    const approvedByLabel = signature.approvedName ? `
      <div style="font-family:${body};font-size:10px;color:${s.textMuted};text-transform:uppercase;letter-spacing:.04em;margin-bottom:8px;">${esc(signatureLabel)}</div>` : '';
    const approval = signature.approvedName ? `
      <div style="font-family:${head};font-weight:600;font-size:13px;color:${s.textDark};margin-top:8px;">${esc(signature.approvedName)}</div>
      <div style="font-family:${body};font-size:11.5px;color:${s.textGray};">${esc(signature.approvedRole)}</div>` : '';
    // Fixed width + margin-left:auto (not flex:1) so this column is always
    // exactly as wide as the signature area itself and always anchored to
    // the right edge of the row - flex:1 previously made it stretch to
    // share the row equally with Bank Details (or take the whole row alone
    // when Bank Details is off), which is what pushed the underline out
    // toward the middle of the page instead of hugging the signature.
    //
    // Order is "Approved By" label -> signature -> underline -> name ->
    // role, not signature -> underline -> label -> name -> role — the
    // label reads as the header for the whole block, sitting above what
    // it's labelling instead of below it.
    signatureHtml = `
      <div style="width:200px;flex-shrink:0;margin-left:auto;text-align:left;">
        ${approvedByLabel}
  ...
  const body = `
    <div style="position:relative;z-index:0;width:714px;min-height:1043px;background:#fff;display:flex;flex-direction:column;">
      ...
      <div style="break-inside:avoid;">
        ...
        <div style="margin-top:22px;">${renderBankSignature(sec.bank, sec.signature, sec.qr, s, doc.docType)}</div>
