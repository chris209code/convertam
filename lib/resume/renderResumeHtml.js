...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* Page margin lives here in CSS, not as a Puppeteer page.pdf() margin
     option — @page :first sets page 1 to zero margin (matching every
     template's current appearance exactly, including mpSidebar's and
     mpHeaderBand's colored panels that are meant to bleed flush to the
     page edge), while the base @page rule gives every OTHER physical page
     real breathing room at the top/bottom. left/right stay 0 on every
     page for all 5 templates — each already draws its own horizontal
     inset via ordinary CSS padding, and mpSidebar's dark column is
     specifically meant to keep bleeding to the left edge even on
     continuation pages (the same way it already does on page 1), not
     suddenly gain a white margin partway through a multi-page CV. A
     Puppeteer JS margin option can't express "different margin on page 1
     vs. later pages" at all — it applies one fixed value to every page —
     which is why this couldn't be fixed that way for the two templates
     with a page-1 bleed design, and is what left it as a known limitation
     in an earlier pass. Confirmed via direct testing that Chromium's
     print engine (the actual @sparticuz/chromium build used here) honors
     @page :first without needing preferCSSPageSize or any page.pdf()
     margin option at all. */
  @page { size: A4; margin: 24px 0 24px 0; }
  @page :first { margin: 0; }
</style>
</head>
<body>
