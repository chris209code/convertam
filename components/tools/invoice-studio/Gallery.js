...
export default function Gallery({ onSelectTemplate, hasSavedDraft, savedDraftTemplateName, onResumeDraft, docType = 'invoice' }) {
  const config = docTypeConfig(docType);

  // Real document content on sample data (the same shape every new document
  // of this type starts from), rendered through the exact same function that
  // produces the live editor's canvas and the downloaded PDF — so a card can
  // never show a template that looks different from what opening it actually
  // gives you. Re-derived per docType so a Quotation's cards show "QUOTATION"
  // labels, not stale "INVOICE" ones.
  const previewHtmlByTemplate = useMemo(() => {
    const previewDoc = emptyDoc('modern', docType);
    const rows = previewDoc.sections.itemsTable.rows;
    const totals = computeInvoiceTotals(rows, previewDoc.discount);
    const wordsText = amountInWords(totals.total, previewDoc.currency);
    const map = {};
    for (const card of TEMPLATE_GALLERY) {
      map[card.id] = renderInvoiceHtml({ ...previewDoc, templateId: card.id }, stylesFor(card.id), totals, wordsText);
    }
    return map;
  }, [docType]);
  ...
  return (
    <div style={{ flex: 1, overflow: 'auto', padding: '32px 48px' }}>
      ...
      <div style={{ fontFamily: 'var(--cs-font-poppins), Poppins, sans-serif', fontWeight: 700, fontSize: 22, marginBottom: 4 }}>{config.label} Templates</div>
