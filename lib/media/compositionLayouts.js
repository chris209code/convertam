...
export function getComposeSize(frameAspect) {
  return FRAME_ASPECTS[frameAspect] || FRAME_ASPECTS.landscape;
}
...
export function drawCompositionFrame(ctx, { timeline, mainEl, overlayEl, rounded = false, border = false }) {
  const { width: w, height: h } = getComposeSize(timeline.frameAspect);
