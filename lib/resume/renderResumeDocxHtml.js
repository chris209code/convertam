...
// addition) is rendered as a separate "Key Achievements" sub-list so the
// Word export keeps the same responsibility/achievement split as the PDF,
// instead of flattening them back into one undifferentiated list.
function expBulletsHtml(exp, fallbackLines) {
  ...
  parts.push(`<p><strong>Key Achievements</strong></p>${bulletsHtml(achievements)}`);
  return parts.join('\n');
}
