...
import { getTrackClips, findActiveClipAt, getTotalDuration, getMasterGain, MAIN_TRACK, clipDuration, BLEND_TRANSITION_TYPES, isOverlayTrackAudible, isSoundTrackAudible, getKenBurnsTransform, getEffectiveClipCrop } from './timeline';
...
import { outroSoundById } from './outroSounds';
...
export async function renderComposedTimeline(timeline, { onStatus, onProgress, cancelToken, captionEvents }) {
  ...
  // Two interchangeable elements rather than one — see the lookahead-preload
  // block in the frame loop below for why: assigning a fresh .src to a
  // <video> element and waiting for loadedmetadata is genuinely slow (tens
  // of ms up to roughly a second, depending on codec/browser), and doing
  // that synchronously at the exact instant of every cut froze the canvas
  // (audio kept running underneath, since Web Audio isn't blocked by this
  // same-thread await) for that long at EVERY clip transition — the
  // "choppy, audio-keeps-going-but-video-freezes-and-skips" playback a
  // multi-clip export produced. Whichever buffer isn't on screen gets the
  // next clip preloaded into it well before the cut arrives.
  const mainVideoElA = document.createElement('video');
  mainVideoElA.playsInline = true;
  const mainVideoElB = document.createElement('video');
  mainVideoElB.playsInline = true;
  ...
  // A signature outro sound (see lib/media/outroSounds.js) — deliberately
  // NOT routed through masterGain: it's the last thing meant to be heard,
  // so it shouldn't itself be quieted by masterFadeOut/masterAudioFadeOut
  // the way every other layer is. Still respects masterVolume/masterMuted
  // (set on its own gain each frame below) since those are genuine "how
  // loud should anything be" controls, not fades.
  const outroSoundConfig = timeline.outroSound?.effectId ? outroSoundById(timeline.outroSound.effectId) : null;
  const outroAudioEl = document.createElement('audio');
  ...
  if (outroSoundConfig) {
    audioCtx.createMediaElementSource(outroAudioEl).connect(outroGain);
    outroAudioEl.src = outroSoundConfig.url;
    await new Promise((resolve) => {
      const timer = setTimeout(resolve, 4000);
      outroAudioEl.onloadedmetadata = () => { clearTimeout(timer); outroDuration = outroAudioEl.duration || 0; resolve(); };
      outroAudioEl.onerror = () => { clearTimeout(timer); resolve(); };
    });
  }
  ...
  // See mainVideoElA/B's own comment above — each buffer keeps its own gain
  // node (both feeding masterGain) and its own audio-routing state, so
  // swapping which one is "on screen" never needs to re-touch Web Audio
  // graph wiring, only which object `activeMainBuf` points at.
  const mainBufA = { videoEl: mainVideoElA, gain: audioCtx.createGain(), srcRef: { current: null }, clipId: null };
  const mainBufB = { videoEl: mainVideoElB, gain: audioCtx.createGain(), srcRef: { current: null }, clipId: null };
  mainBufA.gain.connect(masterGain);
  mainBufB.gain.connect(masterGain);
  let activeMainBuf = mainBufA;
  let preloadMainBuf = mainBufB;
  ...
  // How far ahead of a cut to start preloading the next main-track clip —
  // generous relative to the ~100-900ms load stalls this is meant to
  // absorb, cheap to keep re-checking every frame until it fires once.
  const MAIN_PRELOAD_LOOKAHEAD_SECONDS = 1.5;
  ...
  function handleVisibilityChange() {
    if (document.hidden) {
      ...
      const candidates = [
        mainBufA.videoEl, mainBufB.videoEl, mainCrossfadeVideoEl, mainReplaceAudioEl,
  ...
  await new Promise((resolve) => {
    async function frame() {
      ...
      if (outroSoundConfig && !outroStarted && t >= Math.max(0, totalDuration - outroDuration)) {
        outroStarted = true;
        outroGain.gain.value = timeline.masterMuted ? 0 : (timeline.masterVolume ?? 1) * (timeline.outroSound.volume ?? 0.8);
        outroAudioEl.currentTime = 0;
      ...
      if (mainHit && mainIsImage) {
      ...
      } else if (mainHit && mainHit.clip.id !== activeMainBuf.clipId) {
        if (preloadMainBuf.clipId === mainHit.clip.id) {
          // Already loaded/seeked in the background (see the lookahead
          // block below) — just swap which buffer is "on screen" instead of
          // a fresh, blocking load.
          await preloadMainBuf.preloadPromise;
          const readyBuf = preloadMainBuf;
          preloadMainBuf = activeMainBuf;
          activeMainBuf = readyBuf;
          // The now-former active buffer is still silently playing its old
          // clip until the lookahead below reclaims it for the NEXT one —
          // stop it immediately so its audio doesn't keep bleeding through.
          preloadMainBuf.gain.gain.value = 0;
          preloadMainBuf.videoEl.pause();
          preloadMainBuf.clipId = null;
        } else {
          // Not preloaded in time (a clip shorter than the lookahead
          // window, the very first clip, or a failed preload) — fall back
          // to the original synchronous load, same as before this existed,
          // just on whichever buffer is currently active.
          await loadClipIntoElement(activeMainBuf.videoEl, mainHit.clip, timeline, reversedBlobs);
        }
        ensureAudioRouted(activeMainBuf.videoEl, activeMainBuf.gain, activeMainBuf.srcRef);
        ...
        activeMainBuf.videoEl.currentTime = mainHit.clip.reversed ? mainHit.sourceTime - mainHit.clip.sourceStart : mainHit.sourceTime;
        activeMainBuf.videoEl.playbackRate = mainHit.clip.speed || 1;
        await activeMainBuf.videoEl.play().catch(() => {});
        activeMainBuf.clipId = mainHit.clip.id;
      }

      // Lookahead preload: once the active clip is within
      // MAIN_PRELOAD_LOOKAHEAD_SECONDS of its own end, start loading+seeking
      // the NEXT main-track clip into the other buffer in the background
      // (never awaited here, so it can't block this frame) — by the time
      // the cut above actually arrives, it's usually already ready to swap
      // in instantly instead of paying the load stall right at the cut.
      if (mainHit && !mainIsImage) {
        const remaining = mainHit.clip.start + clipDuration(mainHit.clip) - t;
        if (remaining <= MAIN_PRELOAD_LOOKAHEAD_SECONDS) {
          ...
          if (nextClip && nextSource?.kind !== 'image' && preloadMainBuf.clipId !== nextClip.id) {
            preloadMainBuf.clipId = nextClip.id; // claim now so this doesn't re-trigger every frame while in flight
            const claimedBuf = preloadMainBuf;
            claimedBuf.preloadPromise = (async () => {
              await loadClipIntoElement(claimedBuf.videoEl, nextClip, timeline, reversedBlobs);
              ensureAudioRouted(claimedBuf.videoEl, claimedBuf.gain, claimedBuf.srcRef);
              claimedBuf.gain.gain.value = 0; // stay silent until it's actually swapped in
              claimedBuf.videoEl.currentTime = nextClip.reversed ? 0 : nextClip.sourceStart;
              claimedBuf.videoEl.playbackRate = nextClip.speed || 1;
            })().catch(() => { claimedBuf.clipId = null; }); // a failed preload just falls back to the synchronous path above
          }
        }
      }
      ...
      if (mainHit && mainIsImage) {
        ...
        activeMainBuf.gain.gain.value = ((mainHit.clip.audioMode === 'keep' || mixKeepsOwnAudio) ? 1 : 0) * mainOpacity * (mainHit.clip.gain ?? 1);
      } else {
        mainReplaceGain.gain.value = 0;
      }
      // The inactive buffer must never be heard, whether it's idle, mid
      // preload, or holding a since-passed clip — set unconditionally
      // rather than only in the branches above, so no path can miss it.
      preloadMainBuf.gain.gain.value = 0;
      if (!mainHit || mainIsImage) activeMainBuf.gain.gain.value = 0;
      ...
      for (const layer of overlayLayers) {
        ...
        if (hit && isImage) {
          ...
          // getKenBurnsTransform's own call below) — an overlay-track image
          // clip needs it too, or the "animated zoom/pan" the UI promises
          // silently never happens for a picture-in-picture/overlay image.
          const layerClip = hit.clip.kenBurns && hit.clip.kenBurns !== 'none'
            ? { ...hit.clip, ...getKenBurnsTransform(hit.clip, hit.sourceTime) }
            : hit.clip;
          if (!trackHidden) drawnOverlayLayers.push({ trackId: layer.trackId, el: layer.imageEl, clip: layerClip, opacity });
        } else if (hit) {
      ...
      // Same getEffectiveClipCrop precedence (Smart Reframe, then Ken
      // Burns, then the plain static crop) the live preview loop uses —
      // see VideoEditorWorkspace.js's own copy of this comment for why
      // mediaAspect/targetAspect are only computed for a video main clip.
      let mainCropRenderCtx;
      if (mainHit && !mainIsImage && activeMainBuf.videoEl?.videoWidth) {
        mainCropRenderCtx = { mediaAspect: activeMainBuf.videoEl.videoWidth / activeMainBuf.videoEl.videoHeight, targetAspect: composeWidth / composeHeight };
      }
      drawCompositionFrame(ctx, {
        timeline,
        mainEl: mainHit ? (mainIsImage ? mainImageEl : activeMainBuf.videoEl) : null,
        mainClip: mainHit ? { ...mainHit.clip, ...getEffectiveClipCrop(mainHit.clip, mainHit.sourceTime, mainCropRenderCtx) } : null,
        mainOpacity,
  ...
  await stopped;
  outroAudioEl.pause();
  mainBufA.videoEl.pause();
  mainBufB.videoEl.pause();
  mainCrossfadeVideoEl.pause();
  mainReplaceAudioEl.pause();
  if (mainBufA.videoEl.src) URL.revokeObjectURL(mainBufA.videoEl.src);
  if (mainBufB.videoEl.src) URL.revokeObjectURL(mainBufB.videoEl.src);
...
export async function renderTimeline(timeline, callbacks) {
  ...
  const needsClipEffects = mainClips.some((c) => (c.speed || 1) !== 1 || c.fadeIn > 0 || c.fadeOut > 0 || !isDefaultFilters(c.filters) || c.reversed || (c.gain ?? 1) !== 1 || c.duckBackground || (c.rotation || 0) !== 0 || c.flipH || c.flipV || c.reframe?.mode === 'smart');
  const needsOverlayLayers = timeline.textOverlays.length > 0 || timeline.imageOverlays.length > 0 || (timeline.shapeOverlays || []).length > 0;
  const needsMasterAudio = timeline.masterMuted || (timeline.masterVolume ?? 1) !== 1 || (timeline.masterFadeIn || 0) > 0 || (timeline.masterFadeOut || 0) > 0 || (timeline.masterAudioFadeIn || 0) > 0 || (timeline.masterAudioFadeOut || 0) > 0;
  // A signature outro sound (lib/media/outroSounds.js) is only ever mixed
  // in by the composed path's dedicated outro gain node — the simple/ffmpeg
  // path has no idea it exists, so selecting one has to force the composed
  // path or the sound would be silently dropped from the export.
  const needsOutroSound = !!timeline.outroSound?.effectId;
  ...
  const needsComposed = hasOverlay || needsReframe || needsClipEffects || needsOverlayLayers || needsMasterAudio || needsFitMode || needsGaps || needsImageClips || needsSoundTracks || needsCaptions || needsOutroSound;
