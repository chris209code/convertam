...
function convertImage(file, targetType, quality) {
...
export default function ImageFormatConverterWorkspace() {
  ...
  async function handleConvertAll() {
    ...
    for (let i = 0; i < updated.length; i++) {
      const blob = await convertImage(updated[i].file, targetFormat, 0.92);
      updated[i] = { ...updated[i], outBlob: blob, status: 'done' };
      setItems([...updated]);
    }
    setBusy(false);
  }
    ...
    <div className="panel">
      {items.length === 0 ? (
        <div style={{ border: '2px dashed #CBD5E1', borderRadius: 14, padding: '40px 20px', textAlign: 'center' }}>
          <p style={{ fontSize: '0.95rem', fontWeight: 700, color: '#0F172A', marginBottom: 8 }}>Upload images to convert</p>
          <p style={{ fontSize: '0.82rem', color: '#64748B', marginBottom: 16 }}>Convert between JPG, PNG, and WebP. Upload several at once.</p>
