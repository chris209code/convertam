...
//
// Requires a real Redis-compatible store for that guarantee to hold across
// serverless instances — Vercel's Redis Marketplace integration (Upstash
// under the hood) works out of the box; see .env.local.example for the
// exact env vars it needs. When no store is configured, this falls back to
// an in-memory Map (best-effort within one warm instance only) instead of
// hard-failing the request. An earlier version of this file threw in
// production when unconfigured, on the theory that failing loudly beats
// silently shipping with no dedup protection — in practice that meant
// Paystack had already charged the customer by the time
// /api/payment/verify hit that throw, so every paid conversion started
// failing with the charge already taken and nothing delivered, which is
// far worse than the rare-duplicate-job risk this was guarding against.
// getStore() below still logs loudly (see storeMisconfigWarned) so a
// missing KV store doesn't go unnoticed, it just no longer blocks paying
// customers while it's unfixed.
...
// In-memory fallback, used both in dev and as production's degraded path
// when no real store is configured. Module-level, so it persists across
// requests within one warm process/instance — genuinely not equivalent to
// a shared store in serverless production (separate function instances
// don't share it, and a cold start wipes it), but that's an accepted
// tradeoff here: best-effort dedup beats hard-failing every payment.
...
let storeMisconfigWarned = false;
function getStore() {
  if (HAS_REAL_STORE) return getRedis();
  if (!storeMisconfigWarned) {
    storeMisconfigWarned = true;
    console.error(
      'Payment idempotency store is not configured (missing KV_REST_API_URL/KV_REST_API_TOKEN ' +
      'or UPSTASH_REDIS_REST_URL/UPSTASH_REDIS_REST_TOKEN — see .env.local.example). Falling back ' +
      'to a per-instance in-memory store: paid conversions will still work, but duplicate-job ' +
      'protection is best-effort only until a real store is configured.'
    );
  }
  return null; // callers check for this and use memGet/memSet instead
}
