...
function drawCover(ctx, mediaEl, rect, cropFocus = { x: 0.5, y: 0.5 }) {
  if (!mediaEl) return;
  ...
  if (srcRatio > dstRatio) {
    sh = mediaH;
    sw = sh * dstRatio;
    sx = (mediaW - sw) * cropFocus.x;
    sy = 0;
  } else {
    sw = mediaW;
    sh = sw / dstRatio;
    sx = 0;
    sy = (mediaH - sh) * cropFocus.y;
  }
...
// { ctx, timeline, mainEl, mainClip, mainOpacity, overlayLayers, rounded,
// border, canvasWidth, canvasHeight }. mainEl is always a <video>.
// overlayLayers is an array of { trackId, el, clip, opacity } — el may be a
// <video> or a static <img> (image overlay); clip supplies per-clip filters
// and crop focus; opacity (0..1, default 1) is the fade multiplier from
// getFadeOpacity(). Callers compute each layer's opacity once and pass the
// numbers through rather than this function reaching back into timeline
// state, keeping this module's only dependency on a clip shape minimal.
// Layers are drawn in array order (bottom to top), which callers should
// keep aligned with timeline.overlayTracks' own order.
export function drawCompositionFrame(ctx, { timeline, mainEl, mainClip, mainOpacity = 1, overlayLayers = [], rounded = false, border = false, canvasWidth, canvasHeight }) {
  ...
  function drawInto(mediaEl, rect, clip) {
    ...
    else drawCover(ctx, mediaEl, rect, cropFocus);
  }
...
function drawOneTextOverlay(ctx, o, w, h) {
  if (!o.text) return;
  ctx.save();
  ...
  if (o.background && o.background !== 'none') {
    ...
    let boxX = x - padX;
    if (o.align === 'center') boxX = x - textW / 2 - padX;
    else if (o.align === 'right') boxX = x - textW - padX;
    ctx.fillStyle = o.backgroundColor || '#000000';
    ctx.globalAlpha = (o.opacity ?? 1) * (o.background === 'bar' ? 0.6 : 1);
    ctx.fillRect(boxX, y - o.size / 2 - padY, textW + padX * 2, o.size + padY * 2);
    ctx.globalAlpha = o.opacity ?? 1;
  }
