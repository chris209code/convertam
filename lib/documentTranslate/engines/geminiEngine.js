...
// Output tokens scale with input length (translation output is roughly
// proportional to source length, generally +/-30% depending on language
// pair) — a flat budget either truncates long documents or wastes budget on
// short ones. Clamped to a floor (short texts still get room for the JSON
// wrapper) and the platform's own per-request character cap (see
// limits.js), so this never needs to reach Gemini's absolute maximum.
function estimateOutputTokenBudget(textLength) {
  const estimated = Math.ceil(textLength / 3) + 512;
...
"""`;
  ...
  const { parsed, requestId, usage } = await callGemini({
    ...
    maxOutputTokens: estimateOutputTokenBudget(text.length),
...
${JSON.stringify(blocks)}`;
  ...
  const { parsed, requestId, usage } = await callGemini({
    ...
    maxOutputTokens: estimateOutputTokenBudget(totalLength),
    temperature: 0.1,
    timeoutMs: REQUEST_TIMEOUT_MS,
