...
// further. 15 minutes ≈ 10 sequential chunk calls per transcription — a
// deliberately chosen, still-finite product limit, not a technical wall.
//
// TRANSCRIBE_CHUNK_SECONDS was originally 180 (the Vercel request-size
// ceiling above would allow up to ~280s), but a real transcript came back
// missing several minutes of clearly-spoken audio with NO error and NO
// MAX_TOKENS truncation — Gemini is a general multimodal model, not a
// dedicated streaming ASR engine, and over a long continuous single-speaker
// stretch it can produce a syntactically complete but substantively
// incomplete segment list (fewer segments than were actually spoken)
// without ever signaling truncation. Shorter chunks give it less audio to
// "hold" at once, which empirically reduces how often this happens. The
// remaining risk is caught by transcriptChunking.js's findCoverageGaps +
// geminiTranscription.js's gap-fill pass: after merging, any stretch with
// real audio (checked locally, not by asking Gemini again) but zero
// transcript coverage gets one extra small, targeted re-transcription
// request before this ever reaches a user.
export const TRANSCRIBE_CHUNK_SECONDS = 90; // 1.5 minutes — see comment above
