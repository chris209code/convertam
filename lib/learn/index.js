...
// Reverse lookup for tool pages linking back to Learn: the article that
// names this tool as its `primaryTool` (the article written specifically
// to teach that tool), falling back to the first article that lists it
// under `relatedTools`. Returns null when no article covers this tool yet.
export function getArticleForTool(toolSlug) {
  const primary = ARTICLES.find((a) => a.primaryTool?.slug === toolSlug);
  if (primary) return primary;
  return ARTICLES.find((a) => (a.relatedTools || []).some((t) => t.slug === toolSlug)) || null;
}
