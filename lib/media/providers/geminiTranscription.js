...
import { mergeChunkTranscripts, planTranscribeChunks, validateMergedTranscript, findCoverageGaps } from '../transcriptChunking';
import { detectSilenceInBuffer } from '../silenceDetect';
...
// A gap this short is a plausible natural pause between sentences, not lost
// content — only longer stretches are worth spending an extra Gemini call
// investigating.
const GAP_MIN_SECONDS = 6;
// Extra audio included on each side of a detected gap when re-transcribing
// it, so the patch request doesn't start or end mid-sentence.
const GAP_CONTEXT_PADDING_SECONDS = 2;
// A hard ceiling on how many gap-fill requests one transcription can spend,
// so a pathological file (or a model having a genuinely bad day) can't
// silently balloon into dozens of extra billed Gemini calls.
const MAX_GAP_FILL_ATTEMPTS = 4;

// Is there real speech-or-other-audio signal in this stretch, or is it
// (correctly) uncovered because it's actually silence? Checked locally
// against the audio the browser already decoded — never by asking Gemini
// "was there anything there," which would just repeat whatever mistake
// left the gap in the first place.
function gapHasSignal(audioBuffer, start, end) {
  const rangeSeconds = end - start;
  if (rangeSeconds <= 0) return false;
  const silences = detectSilenceInBuffer(audioBuffer, start, end, { thresholdDb: -40, minSilenceMs: 300 });
  const silentSeconds = silences.reduce((sum, r) => sum + (r.end - r.start), 0);
  return silentSeconds < rangeSeconds * 0.6; // more than 40% of the range has actual signal
}
...
//                    (a post-merge gap-fill request reuses this same phase,
//                    with detail.gapFill: true and chunkIndex/totalChunks
//                    continuing the same sequence, so existing callers that
//                    only render "part X of Y" need no changes)
...
export async function transcribeMedia({ file, onStatus }) {
  ...
  // Gap-fill pass: find any stretch with real audio but zero transcript
  // coverage (see limits.js's TRANSCRIBE_CHUNK_SECONDS comment for why this
  // can happen even with no error and no MAX_TOKENS truncation), and spend
  // one small, targeted re-transcription request per gap before returning —
  // best-effort on top of an already-successful transcription, so a failed
  // patch attempt just leaves the original gap rather than failing the
  // whole result.
  let segments = merged.segments;
  const candidateGaps = findCoverageGaps(segments, durationSeconds, GAP_MIN_SECONDS);
  const realGaps = candidateGaps.filter((g) => gapHasSignal(audioBuffer, g.start, g.end)).slice(0, MAX_GAP_FILL_ATTEMPTS);

  for (let i = 0; i < realGaps.length; i++) {
    const gap = realGaps[i];
    const patchStart = Math.max(0, gap.start - GAP_CONTEXT_PADDING_SECONDS);
    // Clamped to the same per-request duration the server already expects
    // from a regular chunk — a gap can be nearly as wide as a whole chunk
    // (the limit case: an entire chunk produced zero segments), and this
    // patch request must stay inside that same size/duration envelope.
    const patchEnd = Math.min(durationSeconds, gap.end + GAP_CONTEXT_PADDING_SECONDS, patchStart + TRANSCRIBE_CHUNK_SECONDS);
    onStatus?.('transcribing', { chunkIndex: windows.length + i, totalChunks: windows.length + realGaps.length, gapFill: true });

    let patchBlob;
    try {
      ({ blob: patchBlob } = await sliceAndEncodeChunk(audioBuffer, patchStart, patchEnd, TRANSCRIBE_SAMPLE_RATE));
    } catch {
      continue; // best-effort — leave this gap as-is rather than failing an otherwise-successful transcription
    }

    let patchResponse;
    try {
      patchResponse = await transcribeChunkWithRetry({ blob: patchBlob, durationSeconds: patchEnd - patchStart, chunkIndex: windows.length + i, totalChunks: windows.length + realGaps.length, actionId });
    } catch (err) {
      if (err.category === 'rate_limit' || err.category === 'quota_exhausted') break; // no point spending more gap-fill attempts against an exhausted budget
      continue;
    }

    const patchSegments = Array.isArray(patchResponse?.segments) ? patchResponse.segments : [];
    patchSegments.forEach((s) => {
      if (!s || typeof s.text !== 'string' || !Number.isFinite(s.start) || !Number.isFinite(s.end)) return;
      const absStart = patchStart + s.start;
      const absEnd = patchStart + s.end;
      // Outside the actual gap -> this stretch is already owned by a
      // neighboring chunk's own segments; keeping it here too would
      // duplicate real spoken words rather than fill a real gap.
      if (absStart < gap.start || absStart >= gap.end) return;
      segments = [...segments, { start: absStart, end: absEnd, text: s.text }];
    });
  }
  segments.sort((a, b) => a.start - b.start);

  const transcript = normalizeTranscript({ language: merged.language, segments, duration: durationSeconds });
