...
// across roughly `radiusPx` pixels either side of the boundary, instead of
// the sharp step a raw segmentation/brush mask produces. Returns a new
// mask; never mutates the input (callers keep the raw mask for editing).
export function featherMask(mask, width, height, radiusPx) {
  ...
  const blurredH = boxBlur1D(mask, width, height, radius, true);
  const blurredV = boxBlur1D(blurredH, width, height, radius, false);
  const out = new Uint8ClampedArray(mask.length);
  for (let i = 0; i < out.length; i++) out[i] = blurredV[i];
  return out;
}
...
// the two edge-quality passes: featherPx (0-5) softens the mask boundary,
// spillAmount (0-1) controls how strongly the old background's color is
// scrubbed out of the transition band before blending toward the new one.
export function compositeWithBackground(imageData, personMask, bgSpec, edgeOptions = {}) {
  const { width, height, data } = imageData;
  const featherPx = clampNum(edgeOptions.featherPx ?? 0, 0, 5);
  const spillAmount = clampNum(edgeOptions.spillAmount ?? 0, 0, 1);

  const mask = featherPx > 0 ? featherMask(personMask, width, height, featherPx) : personMask;
