...
export async function summarizeDocument({ apiKey, pages, type = 'smart', length = 'medium', focusAreas = [] }) {
  ...
  if (chunks.length <= 1) {
    ...
    const digests = await mapWithConcurrency(chunks, (chunk) => summarizeChunk({ apiKey, chunk, focusAreas }));
...
export async function summarizeChapters({ apiKey, pages, focusAreas = [] }) {
  ...
  const results = await mapWithConcurrency(chapters, (chapter) => summarizeOneChapter({ apiKey, chapter, focusAreas }));
  return { chapters: results, chapterCount: results.length };
}
