...
export async function renderInvoiceToPdf(elements, pdfCtx) {
  ...
  function drawTable(el) {
    ...
    el.rows.forEach((row, i) => {
      ...
      drawText(formatMoney(parseFloat(row.rate) || 0, pdfCtx.currency).replace(pdfCtx.currency, '').trim(), el.x + colRate, textY, { size: 9, color: dark, align: 'right' });
      drawText(`${row.vat || 0}%`, el.x + colVat, textY, { size: 8.5, color: gray, align: 'right' });
      drawText(formatMoney(amount, pdfCtx.currency).replace(pdfCtx.currency, '').trim(), el.x + colAmt, textY, { size: 9.5, f: bold, color: dark, align: 'right' });
  ...
  function drawTotals(el) {
    ...
    drawText(formatMoney(t.subtotal, pdfCtx.currency), el.x + el.w - 16, y, { size: 9.5, color: dark, align: 'right' });
    y += 17;
    drawText('VAT', el.x + 16, y, { size: 9.5, color: gray });
    drawText(formatMoney(t.vat, pdfCtx.currency), el.x + el.w - 16, y, { size: 9.5, color: dark, align: 'right' });
    y += 17;
    if (t.discount > 0) {
      drawText('Discount', el.x + 16, y, { size: 9.5, color: gray });
      drawText(`-${formatMoney(t.discount, pdfCtx.currency)}`, el.x + el.w - 16, y, { size: 9.5, color: dark, align: 'right' });
      y += 17;
    }
    ...
    drawText(formatMoney(t.total, pdfCtx.currency), el.x + el.w - 16, y, { size: 12, f: bold, color: el.useBrand ? brandPrimary : dark, align: 'right' });
  }
  ...
  function drawWatermark(el) {
    ...
    page.drawText(el.content, {
      ...
      size, font: headFont, color: dark, opacity: (el.opacity ?? 12) / 100, rotate: { type: 'degrees', angle: el.rotation ?? 0 },
    });
  }
