...
// Mirrors resumeTemplates.js's ExpBullets: exp.achievements (a CV Improver
// addition) is rendered as a separate "Achievements" sub-list so the Word
// export keeps the same responsibility/achievement split as the PDF,
// instead of flattening them back into one undifferentiated list. A
// heading level (<h3>) rather than another bolded paragraph, plus a blank
// spacer paragraph before it, is the DOCX-safe way to give it the same
// "clearly separate, not just another bullet" treatment the PDF gets via
// spacing/color/a left accent bar — html-to-docx has no CSS margin/border
// support (see file header), but a real heading style and a blank line
// both survive into Word as-is.
function expBulletsHtml(exp, fallbackLines) {
  const achievements = (exp.achievements || []).filter(Boolean);
  if (!achievements.length) return bulletsHtml(fallbackLines);
  const parts = [];
  if (fallbackLines.length) parts.push(`<p><strong>Responsibilities</strong></p>${bulletsHtml(fallbackLines)}`);
  parts.push(`<p>&nbsp;</p><h3>Achievements</h3>${bulletsHtml(achievements)}`);
  return parts.join('\n');
}
...
function modernAchievementsHtml(achievements = []) {
  return bulletsHtml(achievements);
}
