...
export const LEAKAGE_SENTENCE_START_RE = new RegExp(`^(${LEAKAGE_VERBS.join('|')})\\b`, 'i');
...
export const PLACEHOLDER_RE = /\[[^\]]{1,80}\]/;
...
// paragraph in the fallback-strip step. Exported so other AI-output
// validators (e.g. LinkedIn Optimizer) can reuse the exact same splitting
// and leakage rules instead of re-deriving them.
export function splitSentences(text) {
  return (text.match(/[^.!?]+[.!?]+|[^.!?]+$/g) || [text]).map((s) => s.trim()).filter(Boolean);
}

export function textHasLeakage(text) {
...
export function detectCvLeakage(structured) {
  ...
  (structured.experience || []).forEach((exp, i) => {
    ...
    (exp.suggestedAchievements || []).forEach((b) => {
      if (textHasLeakage(b)) issues.push(`experience[${i}] suggested achievement contains instruction-like or placeholder text: "${b}"`);
    });
  });
...
export function stripCvLeakage(structured) {
  ...
  cleaned.experience = (cleaned.experience || []).map((exp) => ({
    ...
    suggestedAchievements: (exp.suggestedAchievements || []).filter((b) => !textHasLeakage(b)),
  }));
