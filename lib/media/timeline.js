...
import { getSmartReframeTransform } from './smartReframe';
...
export function createTimeline() {
  return {
    ...
    masterMuted: false,
    masterFadeIn: 0, // seconds — fades the WHOLE mix up from silence at the very start of the timeline, together with the picture fading up from black (see drawMasterFade)
    masterFadeOut: 0, // seconds — same, down to silence at the very end, together with the picture fading to black
    masterAudioFadeIn: 0, // seconds — audio-only fade up from silence at the very start; independent of masterFadeIn, the picture is untouched
    masterAudioFadeOut: 0, // seconds — audio-only fade down to silence at the very end, so a clip's audio doesn't just cut off — independent of masterFadeOut, the picture is untouched
    // A short signature sound effect (see lib/media/outroSounds.js) that
    // plays right at the very end of the timeline — id null means none.
    // Independent of every other audio control here: it always plays at
    // its own fixed volume (scaled only by masterVolume/masterMuted, never
    // by masterFadeOut/masterAudioFadeOut) since it's the LAST thing meant
    // to be heard, not something that should itself fade away.
    outroSound: { effectId: null, volume: 0.8 },
    ...
    // ensureProjectAudioTrack) used as the independent "Audio Track" bed —
    // built for Replace Video/Sync Audio (an already-recorded or edited
    // audio track paired with a separately-chosen visual source of a
    // different length) and reusable as a plain "add background music"
    // track for any timeline, including a multi-image slideshow. Just an
    // ordinary sound track once created — split/trim/move/delete its
    // clip(s) exactly like any other clip — this field only exists so the
    // UI can recognize and label "the" Audio Track rather than treating it
    // as one more anonymous entry in Sound clips. null until first used.
    projectAudioTrackId: null,
    ...
    captionStyle: {
      ...
      bold: true,
      color: '#FFFFFF',
      highlightColor: '#FFD400', // the active word's color in 'word'/'karaoke' highlightMode, when highlightColorMode is 'fixed'
      highlightColorMode: 'fixed', // 'fixed' | 'random' — 'random' ignores highlightColor and gives each word its own random bright color instead (see compositionLayouts.js's drawCaptions)
...
export function getMasterGain(timeline, timeSeconds) {
  ...
  // Audio-only fade, independent of the picture-fade-to-black pair above —
  // for when a clip's audio should ease out instead of cutting off abruptly
  // at the very last frame, without the picture itself dipping to black.
  // Multiplying (rather than picking whichever is "more restrictive") is
  // what lets both kinds of fade compose correctly if a project happens to
  // use both at once, and is a no-op (×1) whenever one isn't set.
  const audioFadeIn = timeline.masterAudioFadeIn || 0;
  const audioFadeOut = timeline.masterAudioFadeOut || 0;
  if (audioFadeIn > 0 && timeSeconds < audioFadeIn) gain *= Math.max(0, timeSeconds / audioFadeIn);
  if (audioFadeOut > 0 && timeSeconds > totalDuration - audioFadeOut) gain *= Math.max(0, (totalDuration - timeSeconds) / audioFadeOut);
  return gain;
}
...
export function addClip(timeline, sourceId, track = MAIN_TRACK) {
  ...
  const clip = {
    ...
    kenBurns: 'none', // 'none' | 'zoom-in' | 'zoom-out' | 'pan-left' | 'pan-right' | 'pan-up' | 'pan-down' — animates cropFocus/cropZoom over the clip's own duration instead of the static manual crop values; meant for image clips (a still has no motion of its own), see getKenBurnsTransform()
    reframe: null, // null | {mode:'smart', subjectMode, follow, smoothing, trackingData, offset, fallbackReason} — see setClipReframeMode()/getEffectiveClipCrop() below; null means "not using Smart Reframe," same as kenBurns:'none' meaning "not animating"
  };
...
// Ripple only ever applies to the strictly-sequential main track, and to
// the one reserved linked-audio track that exists purely to mirror
// MAIN_TRACK's own sequencing 1:1 (see linkVideoClipAudio) — without this,
// deleting a video clip mid-sequence would ripple-close the gap on the
// video while its still-linked LATER audio clips stayed at their old,
// now-mismatched positions. Every other track — overlay, Sound clips, the
// Audio Track — is free-form and manually positioned: a clip only ever
// moves because the user dragged it, never as a side effect of deleting
// or resizing some OTHER clip nearby.
function isRippleTrack(timeline, track) {
  return track === MAIN_TRACK || track === timeline.mainAudioTrackId;
}

// Ripples every later same-track clip by `delta` (a duration change's exact
// timeline-seconds shift) as ONE uniform shift, rather than clamping each
// clip's own new start independently — clamping per-clip can collapse two
// or more later clips onto the exact same point (0) whenever a large enough
// negative delta would send several of their starts negative at once,
// silently creating a real overlap every other operation on this track
// guards against. Clamping the delta itself once, based on the EARLIEST
// later clip, preserves every later clip's relative spacing instead — the
// same approach moveClips already uses for a group drag.
function rippleLaterClips(timeline, track, excludeClipId, fromStart, delta) {
  if (Math.abs(delta) < 1e-9) return timeline;
  const laterIds = new Set(timeline.clips.filter((c) => c.track === track && c.id !== excludeClipId && c.start >= fromStart - 1e-6).map((c) => c.id));
  if (!laterIds.size) return timeline;
  const minLaterStart = Math.min(...timeline.clips.filter((c) => laterIds.has(c.id)).map((c) => c.start));
  const clampedDelta = minLaterStart + delta < 0 ? -minLaterStart : delta;
  return { ...timeline, clips: timeline.clips.map((c) => (laterIds.has(c.id) ? { ...c, start: c.start + clampedDelta } : c)) };
}

export function deleteClip(timeline, clipId) {
  const clip = timeline.clips.find((c) => c.id === clipId);
  if (!clip) return timeline;
  if (!isRippleTrack(timeline, clip.track)) {
...
// Bulk delete for multi-select — same ripple-track rule as deleteClip,
// just summing every deleted clip's own contribution to how far a
// surviving clip on a ripple track shifts left (only counting deleted
// clips that end at or before that survivor's own start), so it's a
// single atomic pass rather than deleting one at a time and re-deriving
// shifts in between. Every other track's clips are removed in place,
// untouched.
export function deleteClips(timeline, clipIds) {
  ...
  for (const c of toDelete) {
    if (!isRippleTrack(timeline, c.track)) continue;
...
// setClipSpeed alone leaves every other clip's `start` exactly where it
// was — fine for a clip with nothing after it, but a speed change resizes
// this clip's own on-timeline duration (clipDuration = sourceSpan/speed),
// so anything sitting right after it would otherwise get silently
// overlapped (sped up) or left with a new gap (slowed down) with no
// warning, unlike every other edit on this track which actively guards
// against overlap. Ripples every later clip on the SAME track by the
// exact delta this clip's end just moved by, so a back-to-back sequence
// stays back-to-back — matching ripple-trim behavior in a normal NLE —
// while any gap the user deliberately left further down the track is
// preserved (everything past this clip's old end moves together).
export function setClipSpeedRipple(timeline, clipId, speed) {
  const clip = timeline.clips.find((c) => c.id === clipId);
  if (!clip) return timeline;
  const next = setClipSpeed(timeline, clipId, speed);
  // Same ripple-track rule as deleteClip's own guard (see isRippleTrack):
  // only the main track and its mirrored linked-audio track ripple a
  // duration change onto later clips. Every other track is free-form and
  // manually positioned — a clip only moves because the user dragged it,
  // never as a side effect of resizing some OTHER clip nearby.
  if (!isRippleTrack(timeline, clip.track)) return next;
  const oldEnd = clip.start + clipDuration(clip);
  const updated = next.clips.find((c) => c.id === clipId);
  const delta = (updated.start + clipDuration(updated)) - oldEnd;
  return rippleLaterClips(next, clip.track, clipId, oldEnd, delta);
}

// Per-clip duration control for a single image clip — a static image has
// no natural "speed" the way setClipSpeedRipple's slider does, but the
// same "resize this clip, then ripple later clips on a sequential track by
// however much its end moved" mechanics apply identically. Images always
// start at sourceStart 0 (see handleMainImageFiles), so `seconds` IS the
// clip's whole on-screen duration.
export function setImageClipDuration(timeline, clipId, seconds) {
  const clip = timeline.clips.find((c) => c.id === clipId);
  if (!clip) return timeline;
  const dur = Math.max(0.1, seconds);
  const oldEnd = clip.start + clipDuration(clip);
  const next = trimClip(timeline, clipId, { sourceStart: 0, sourceEnd: dur });
  if (!isRippleTrack(timeline, clip.track)) return next;
  const updated = next.clips.find((c) => c.id === clipId);
  const delta = (updated.start + clipDuration(updated)) - oldEnd;
  return rippleLaterClips(next, clip.track, clipId, oldEnd, delta);
...
const VALID_KEN_BURNS = ['none', 'zoom-in', 'zoom-out', 'pan-left', 'pan-right', 'pan-up', 'pan-down'];
export function setClipKenBurns(timeline, clipId, mode) {
  const normalized = VALID_KEN_BURNS.includes(mode) ? mode : 'none';
  return { ...timeline, clips: timeline.clips.map((c) => (c.id === clipId ? { ...c, kenBurns: normalized } : c)) };
}

// How far zoomed in a 'zoom-in'/'zoom-out' animation ends/starts, and how
// zoomed in a 'pan-*' animation stays throughout — needs enough margin
// that panning the crop window never reveals past the image's own edge.
const KEN_BURNS_ZOOM = 1.18;
// How far off-center a pan's cropFocus travels in each direction — kept
// well inside 0..1 so the crop window (sized by KEN_BURNS_ZOOM above)
// never runs past the image's edge at either end of the pan.
const KEN_BURNS_PAN_MARGIN = 0.18;

// Pure function: clip + elapsed seconds within it -> the {cropFocus, cropZoom}
// drawCover() should use THIS frame. Reuses the exact same fields as the
// static manual crop control (cropFocus/cropZoom) rather than adding a
// parallel drawing path — Ken Burns is just those two values animated over
// the clip's own duration instead of held constant. Callers can call this
// unconditionally: 'none' (or missing) just echoes the clip's static values
// back, so there's no separate "is Ken Burns on" branch needed at the call
// site — see VideoEditorWorkspace.js's live preview and timelineRender.js's
// composed export, which both do the same override right before handing
// mainClip to drawCompositionFrame.
export function getKenBurnsTransform(clip, elapsedSeconds) {
  const mode = clip.kenBurns || 'none';
  if (mode === 'none') return { cropFocus: clip.cropFocus || { x: 0.5, y: 0.5 }, cropZoom: clip.cropZoom || 1 };
  const duration = Math.max(0.1, clipDuration(clip));
  const progress = Math.max(0, Math.min(1, elapsedSeconds / duration));
  if (mode === 'zoom-in') return { cropFocus: { x: 0.5, y: 0.5 }, cropZoom: 1 + (KEN_BURNS_ZOOM - 1) * progress };
  if (mode === 'zoom-out') return { cropFocus: { x: 0.5, y: 0.5 }, cropZoom: KEN_BURNS_ZOOM - (KEN_BURNS_ZOOM - 1) * progress };
  const swing = KEN_BURNS_PAN_MARGIN * 2 * progress;
  if (mode === 'pan-left') return { cropFocus: { x: 0.5 + KEN_BURNS_PAN_MARGIN - swing, y: 0.5 }, cropZoom: KEN_BURNS_ZOOM };
  if (mode === 'pan-right') return { cropFocus: { x: 0.5 - KEN_BURNS_PAN_MARGIN + swing, y: 0.5 }, cropZoom: KEN_BURNS_ZOOM };
  if (mode === 'pan-up') return { cropFocus: { x: 0.5, y: 0.5 + KEN_BURNS_PAN_MARGIN - swing }, cropZoom: KEN_BURNS_ZOOM };
  if (mode === 'pan-down') return { cropFocus: { x: 0.5, y: 0.5 - KEN_BURNS_PAN_MARGIN + swing }, cropZoom: KEN_BURNS_ZOOM };
  return { cropFocus: clip.cropFocus || { x: 0.5, y: 0.5 }, cropZoom: clip.cropZoom || 1 };
}

// ---- Smart Reframe — see lib/media/smartReframe.js for the actual
// analysis/geometry. Everything here just manages the clip's own
// reframe metadata, following the same shape/precedent as kenBurns above:
// a null/'off' mode means "not in use," and turning it on doesn't discard
// any previously analyzed data, so switching back and forth is cheap. ----

export function setClipReframeMode(timeline, clipId, mode) {
  return {
    ...timeline,
    clips: timeline.clips.map((c) => {
      if (c.id !== clipId) return c;
      if (mode !== 'smart') return { ...c, reframe: c.reframe ? { ...c.reframe, mode: 'off' } : null };
      return { ...c, reframe: { subjectMode: 'auto', follow: true, smoothing: 'balanced', trackingData: [], offset: { x: 0, y: 0 }, fallbackReason: null, ...c.reframe, mode: 'smart' } };
    }),
  };
}
export function setClipReframeSubjectMode(timeline, clipId, subjectMode) {
  return { ...timeline, clips: timeline.clips.map((c) => (c.id === clipId && c.reframe ? { ...c, reframe: { ...c.reframe, subjectMode } } : c)) };
}
export function setClipReframeSmoothing(timeline, clipId, smoothing) {
  return { ...timeline, clips: timeline.clips.map((c) => (c.id === clipId && c.reframe ? { ...c, reframe: { ...c.reframe, smoothing } } : c)) };
}
export function setClipReframeFollow(timeline, clipId, follow) {
  return { ...timeline, clips: timeline.clips.map((c) => (c.id === clipId && c.reframe ? { ...c, reframe: { ...c.reframe, follow } } : c)) };
}
// Called once analysis (see analyzeClipForSmartReframe) finishes — stores
// the result whether or not a subject was actually found, so the UI can
// show the "couldn't detect a subject, centered automatically" message
// (fallbackReason) without re-running analysis to find that out again.
export function setClipReframeAnalysis(timeline, clipId, { trackingData, mediaAspect, fallbackReason }) {
  return {
    ...timeline,
    clips: timeline.clips.map((c) => (c.id === clipId && c.reframe ? { ...c, reframe: { ...c.reframe, trackingData, mediaAspect, fallbackReason } } : c)),
  };
}
// The manual-override offset (see getSmartReframeTransform) — dragging the
// crop pad while Smart Reframe is active adjusts THIS instead of
// overwriting cropFocus directly, so the analyzed tracking path is never
// destroyed by a manual nudge.
export function setClipReframeOffset(timeline, clipId, offset) {
  return {
    ...timeline,
    clips: timeline.clips.map((c) => (c.id === clipId && c.reframe
      ? { ...c, reframe: { ...c.reframe, offset: { x: Math.max(-1, Math.min(1, offset.x)), y: Math.max(-1, Math.min(1, offset.y)) } } }
      : c)),
  };
}
// "Reset" control (spec: "Return to the automatically calculated
// framing") — clears only the manual offset, keeps the analyzed path.
export function resetClipReframeOffset(timeline, clipId) {
  return setClipReframeOffset(timeline, clipId, { x: 0, y: 0 });
}

// The one function both the live preview loop and the composed export
// loop call to get this frame's crop, in place of their previous direct
// getKenBurnsTransform-or-static-fallback ternary — same precedence rule
// Ken Burns already established (an active override wins, otherwise fall
// through), with Smart Reframe checked first since it's the more specific
// per-clip choice. `renderCtx` carries the two numbers Smart Reframe needs
// that Ken Burns doesn't (mediaAspect, targetAspect) — see
// getSmartReframeTransform in smartReframe.js for why only the ratios
// matter, never absolute pixel sizes.
export function getEffectiveClipCrop(clip, sourceTimeSeconds, renderCtx) {
  if (clip.reframe?.mode === 'smart') {
    const smart = getSmartReframeTransform(clip, sourceTimeSeconds, renderCtx || {});
    if (smart) return smart;
  }
  if (clip.kenBurns && clip.kenBurns !== 'none') return getKenBurnsTransform(clip, sourceTimeSeconds);
  return { cropFocus: clip.cropFocus || { x: 0.5, y: 0.5 }, cropZoom: clip.cropZoom || 1 };
}
...
// Same idea as setMasterFade, for the audio-only fade pair — kept as a
// separate setter (rather than folding into setMasterFade) so the two
// concerns can't be confused at the call site either.
export function setMasterAudioFade(timeline, { masterAudioFadeIn, masterAudioFadeOut }) {
  return {
    ...timeline,
    masterAudioFadeIn: masterAudioFadeIn !== undefined ? Math.max(0, masterAudioFadeIn) : timeline.masterAudioFadeIn,
    masterAudioFadeOut: masterAudioFadeOut !== undefined ? Math.max(0, masterAudioFadeOut) : timeline.masterAudioFadeOut,
  };
}

export function setOutroSound(timeline, { effectId, volume }) {
  return {
    ...timeline,
    outroSound: {
      effectId: effectId !== undefined ? effectId : timeline.outroSound?.effectId ?? null,
      volume: volume !== undefined ? Math.max(0, Math.min(1, volume)) : timeline.outroSound?.volume ?? 0.8,
    },
  };
}
...
export function removeSoundTrack(timeline, trackId) {
  return {
    ...
    projectAudioTrackId: timeline.projectAudioTrackId === trackId ? null : timeline.projectAudioTrackId,
  };
}

// Returns { timeline, trackId } (same convention as ensureMainAudioTrack) —
// creates the reserved "Audio Track" sound track the first time it's
// needed, then reuses it. Once created it's an ordinary sound track in
// every respect (split/trim/move/delete its clip(s) the same generic way
// as any other) — this flag only exists so the UI can find and label it.
export function ensureProjectAudioTrack(timeline) {
  if (timeline.projectAudioTrackId != null && timeline.soundTracks.some((t) => t.id === timeline.projectAudioTrackId)) {
    return { timeline, trackId: timeline.projectAudioTrackId };
  }
  const { timeline: withTrack, trackId } = addSoundTrack(timeline);
  return { timeline: { ...withTrack, projectAudioTrackId: trackId }, trackId };
}
...
export function moveLinkedClipToIndex(timeline, track, videoClipId, newIndex) {
  const next = moveClipToIndex(timeline, track, videoClipId, newIndex);
  if (next === timeline) return next;
  // moveClipToIndex repacks the WHOLE track back-to-back using each clip's
  // own duration — running it separately on the linked audio track would
  // repack using THAT track's own durations instead, which only matches the
  // video track's result the whole way through if every linked pair's speed
  // (and therefore duration) still happens to match. Mirroring each
  // main-track clip's own new start directly onto its linked audio clip
  // keeps the two tracks in sync regardless — the same "linked clips share
  // a start" invariant moveLinkedClip enforces via delta for a single clip.
  const mainClips = getTrackClips(next, track);
  return {
    ...next,
    clips: next.clips.map((c) => {
      const linkedVideo = mainClips.find((v) => v.id === c.linkedVideoClipId);
      return linkedVideo ? { ...c, start: linkedVideo.start } : c;
    }),
  };
}
...
export function duplicateLinkedClip(timeline, videoClipId) {
  ...
  next = afterAudioDup;
  const newAudioClip = next.clips[next.clips.length - 1];
  // duplicateClip positions each copy off its OWN clipDuration — fine when
  // video and audio speeds match, but a linked pair with independently
  // adjusted speeds (see setClipSpeedRipple's Apply to Audio only) has
  // different durations, so the two copies would otherwise land at
  // different starts. Force the audio copy back onto the video copy's own
  // start — same "linked clips share a start" invariant moveLinkedClip
  // enforces via delta — falling back to wherever duplicateClip already put
  // it if that exact spot is already taken on the audio track.
  const resynced = moveClip(next, newAudioClip.id, newVideoClip.start);
  if (resynced !== next) next = resynced;
  return { ...next, clips: next.clips.map((c) => (c.id === newAudioClip.id ? { ...c, linkedVideoClipId: newVideoClip.id } : c)) };
}
...
export function removeSource(timeline, sourceId) {
  ...
  return {
    ...
    projectAudioTrackId: liveOverlayTrackIds.has(timeline.projectAudioTrackId) ? timeline.projectAudioTrackId : null,
  };
}
...
export function findActiveClipAt(timeline, track, timelineSeconds) {
  ...
  for (const clip of trackClips) {
    ...
    if (timelineSeconds >= clip.start && timelineSeconds < clip.start + dur) {
      ...
      // A static image has no source position to seek a decoder to, but
      // "elapsed time within this clip" is still a real, meaningful value
      // for it (an image's own sourceStart is always 0, per addClip/
      // trimClip) — every fadeIn/fadeOut/crossfade-tail calculation reads
      // `sourceTime - clip.sourceStart` as "how far into this clip's
      // on-screen display are we," and a permanently-0 sourceTime here
      // made that permanently read as "just started," which silently
      // broke every fade and blend transition touching an image clip
      // (fadeOut's own trigger condition could never become true, and any
      // fadeIn left the image stuck at 0 opacity for its whole duration).
      return { clip, sourceTime: isStatic ? (timelineSeconds - clip.start) : clip.sourceStart + (timelineSeconds - clip.start) * (clip.speed || 1) };
    }
  }
  return null;
}
...
// Produces a NEW standalone timeline containing only the [rangeStart,
// rangeEnd) window of this one, re-based so rangeStart becomes 0 — used by
// the AI Highlights panel's "Clip this" action. Deliberately reuses the
// exact same clip fields every other timeline operation already
// understands (start/sourceStart/sourceEnd/speed) rather than inventing a
// parallel "export window" concept, so every existing preview/render/export
// code path handles the result with zero changes of its own.
export function extractTimelineRange(timeline, rangeStart, rangeEnd) {
  const start = Math.max(0, rangeStart);
  const end = Math.max(start + 0.1, rangeEnd);
  ...
  const clips = timeline.clips
    .map((c) => {
      ...
      const clipEnd = c.start + dur;
      if (clipEnd <= start || clipStart >= end) return null; // fully outside the kept window — drop
      let sourceStart = c.sourceStart;
      let sourceEnd = c.sourceEnd;
      if (clipStart < start) sourceStart = c.sourceStart + (start - clipStart) * speed;
      if (clipEnd > end) sourceEnd = c.sourceEnd - (clipEnd - end) * speed;
      sourceEnd = Math.max(sourceStart + 0.02, sourceEnd); // guard against a degenerate/negative range from float rounding
      return { ...c, start: Math.max(0, clipStart - start), sourceStart, sourceEnd };
    })
    .filter(Boolean);

  // A clip whose linked audio/video counterpart got fully dropped by the
  // trim no longer has anything to stay in sync with.
  const keptClipIds = new Set(clips.map((c) => c.id));
  const rebasedClips = clips.map((c) =>
    (c.linkedVideoClipId && !keptClipIds.has(c.linkedVideoClipId)) ? { ...c, linkedVideoClipId: null } : c
  );

  // Text/image/shape overlays share the same {start, end (null = "through
  // the end of the timeline")} convention, so one helper covers all three.
  const rebaseTimed = (arr) =>
    (arr || [])
      .filter((o) => (o.end == null || o.end > start) && o.start < end)
      .map((o) => ({ ...o, start: Math.max(0, o.start - start), end: o.end == null ? null : Math.max(0, o.end - start) }));

  return {
    ...timeline,
    clips: rebasedClips,
    textOverlays: rebaseTimed(timeline.textOverlays),
    imageOverlays: rebaseTimed(timeline.imageOverlays),
    shapeOverlays: rebaseTimed(timeline.shapeOverlays),
