...
// Bright, highly-saturated palette for the word/karaoke highlight's "random
// multicolor" mode — deliberately not an ordered rainbow (that would read as
// a deliberate sweep rather than "random").
const RANDOM_HIGHLIGHT_COLORS = ['#FF3B30', '#FFD400', '#32ADE6', '#FF2D95', '#4CD964', '#AF52DE', '#FF9500', '#00E5C7', '#FF6482', '#5E5CE6'];

// Deterministic per-word color pick — hashed from the word's own text+start
// time rather than Math.random(), so it stays the SAME color across every
// frame this word is drawn (no flicker) while still landing on a different
// palette slot than most neighboring words.
function hashWordToColor(word) {
  const str = `${word.text}|${word.start.toFixed(3)}`;
  let hash = 0;
  for (let i = 0; i < str.length; i++) hash = (hash * 31 + str.charCodeAt(i)) | 0;
  return RANDOM_HIGHLIGHT_COLORS[Math.abs(hash) % RANDOM_HIGHLIGHT_COLORS.length];
}
...
export function drawCaptions(ctx, { style, event, timelineSeconds, timeline, canvasWidth, canvasHeight }) {
  ...
  lines.forEach((line, li) => {
    ...
    for (const word of line) {
      ...
      const isRandomHighlight = style.highlightColorMode === 'random';
      if (style.highlightMode === 'word' && isActive) color = isRandomHighlight ? hashWordToColor(word) : (style.highlightColor || '#FFD400');
      else if (style.highlightMode === 'karaoke' && (isActive || isPast)) color = isRandomHighlight ? hashWordToColor(word) : (style.highlightColor || '#FFD400');
      if (style.outline) {
...
// Destination rect for one image overlay (logo/watermark) — the single
// source of truth for both drawing and the drag-to-reposition hit-testing,
// same "forward math and inverse math can never drift apart" reasoning as
// getPipRange/pipPositionFromPoint above.
export function getImageOverlayBounds(o, img, w, h) {
...
export function drawImageOverlays(ctx, { timeline, timelineSeconds, imageElements, canvasWidth, canvasHeight }) {
  ...
  for (const o of timeline.imageOverlays) {
    ...
    const b = getImageOverlayBounds(o, img, w, h);
    if (!b) continue;
    ctx.save();
...
// A project-level fade to/from black — the SAME masterFadeIn/masterFadeOut
// seconds already driving the whole mix's audio gain (see timeline.js's
// getMasterGain) also drive this visual overlay, since "fade out" at the
// end of a whole video conventionally darkens picture and sound together,
// not just the sound. Drawn last, over everything (including the
// watermark and captions), so the final seconds actually go black rather
// than just dim behind still-visible content.
export function drawMasterFade(ctx, { timeline, timelineSeconds, totalDuration, canvasWidth, canvasHeight }) {
  const fadeIn = timeline.masterFadeIn || 0;
  const fadeOut = timeline.masterFadeOut || 0;
  if (fadeIn <= 0 && fadeOut <= 0) return;
  let opacity = 1;
  if (fadeIn > 0 && timelineSeconds < fadeIn) opacity = Math.min(opacity, timelineSeconds / fadeIn);
  if (fadeOut > 0 && timelineSeconds > totalDuration - fadeOut) opacity = Math.min(opacity, Math.max(0, (totalDuration - timelineSeconds) / fadeOut));
