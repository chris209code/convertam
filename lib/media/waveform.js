...
function peaksFromChannelData(channelData, peaksCount) {
...
export async function extractWaveformPeaks(file, peaksCount = 200) {
  ...
  return peaksFromChannelData(audioBuffer.getChannelData(0), peaksCount);
}

// Same bucketing, but for an AudioBuffer already held in memory (a freshly
// recorded take, or a source decoded once and reused across many clips) —
// avoids a redundant decode/file round-trip when the caller already has the
// buffer, which Recording Studio's multitrack model always does.
export function peaksFromAudioBuffer(audioBuffer, peaksCount = 200) {
  return peaksFromChannelData(audioBuffer.getChannelData(0), peaksCount);
}
